...
> **Current operational state.** The desk's trading timers are **paused by owner decision** (since roughly 2026-09-03) while the trade logic is stabilised. Everything below describes how the schedule is installed and how it behaves when running; it does not describe a desk that is currently placing orders. Automatic rotation (`execution.rotation_enabled: true`) has consequently never executed against a live broker.
