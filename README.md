...
> **Current operational state.** The desk's trading timers are **paused by owner decision** (since roughly 2026-09-03), and as of 2026-09-14 that pause is reboot-proof: all six session timers (`quant-agent-{morning,midday,close,intra_check,evening,earnings_preprocess}.timer`) are `systemctl --user disable`d, not merely left unstarted, so a box restart cannot silently resume trading. Each is declared in `scripts/systemd/paused_units.yaml`, which `scripts/check_unit_drift.py` reads so the daily drift check reports them as a deliberate pause rather than alarming on six units it expects to be enabled. Everything below describes how the schedule is installed and how it behaves when running; it does not describe a desk that is currently placing orders. Automatic rotation (`execution.rotation_enabled: true`) has consequently never executed against a live broker. **To restart the desk:** `systemctl --user enable --now` the six timers, and remove their entries from `scripts/systemd/paused_units.yaml` in the same change — leaving a re-enabled unit on the paused list is itself flagged as drift (list and box disagreeing).
