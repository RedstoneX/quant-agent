...
- Sector concentration (gross, cumulative including pending same-sector buys) — **a dial, not a gate** (spec §10.3, owner-ratified 2026-09-01). 40% is the diversification *target*: crossing it no longer refuses the trade, it SHRINKS it. `PortfolioConstructor` scales each further trade in that sector by `(60 − sector%) / (60 − 40)`, so a sector at 50% takes the next idea at half size and one at 55% takes it at a quarter — a high-conviction opportunity in a crowded sector is taken, smaller, because "each trade opportunity is an opportunity on its own." Breaching 40% emits an *advisory* `max_sector_pct` violation the Risk Manager sees. The hard block (`max_sector_hard_pct`, in `HARD_BLOCK_RULES`) sits at the **60% absolute ceiling** — 1.5× the target, where an ordinary 20% sector drawdown would cost 12% of equity, four times the daily-loss breaker. A trade shrunk below `cash_sweep.min_order_usd` ($500) is refused outright rather than placed as a token position: it would pay full commission and consume a slot for an immaterial payoff. Both thresholds are configurable (`risk.max_sector_pct` / `risk.max_sector_hard_pct`); the ceiling defaults to 1.5× the target when unset.
- Stop loss required
