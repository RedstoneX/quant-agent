...
- **A stalled entry is repriced, then escalated — never dropped silently** (2026-09-11, `execution.repeg_enabled`, still OFF by default): a limit BUY that drifts out of reach used to be cancelled by price protection and that was the end of it — correct, but a silent lost opportunity (6 of 68 historical proposals). The bounded re-peg walks the working limit UP toward the market instead, using Alpaca's real **Replace Order** endpoint (`PATCH /v2/orders/{id}`) so it stays ONE order rather than being cancelled and re-submitted as a new one. Bounded three ways, all hard: never past `max_entry_slippage_bps` above the reference captured at submission; never more than `repeg_max_attempts` replacements; and never longer in total than `repeg_max_attempts × repeg_poll_seconds`, now an enforced monotonic deadline rather than a docstring claim. **Replaces are strictly one at a time** — Alpaca refuses to replace an order still sitting in `pending_replace`, so `AlpacaBroker.await_replacement_confirmed` waits for the superseded order to report `replaced` on the `trade_updates` websocket (the same real-time machinery as the order-fill wait, PR #287, with its REST fallback intact) before the next replacement is allowed; an unconfirmed swap stops the chase rather than racing broker timing. When the chase runs out of attempts, time, or price room and the entry still has not filled, the owner gets a **standalone Telegram alert** (`send_owner_alert`, the same out-of-band path as a naked position) naming the symbol, the prices tried, the ceiling, and the outcome. A fill, a partial fill, or a market that never moved against us is silent. The order is **left working, not cancelled**, and nothing is auto-resubmitted — automatic resubmission of a brand-new order after the original dies is a deliberate open owner decision, not an omission (see the comment above `_alert_owner_repeg_exhausted` in `src/pipeline_stages.py`).
