...
- **Your stop sits anywhere else** → it is pushed out to a deterministic minimum distance (`risk.min_stop_atr_multiple`, base **2.5** ATRs since 2026-09-10, scaled by setup and macro regime — reachable range 2.14–3.00 ATRs). On a `range` setup reward:risk is then measured against THAT widened stop; a result under 1.5 is **not a rejection and not a size-cap** — the trade ships at the size PM asked for and is ranked on the real ratio — and only a ratio that cannot be measured at all is refused. On a `breakout` no reward:risk is measured on either stop. A stop nothing on the chart defends does not earn the exemption.
