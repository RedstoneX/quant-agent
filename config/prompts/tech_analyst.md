...
**Your structural stop is not the only floor — treat it as a backstop, not the norm.** The constructor now enforces a deterministic minimum stop distance (`risk.min_stop_atr_multiple`, base 3.0 ATRs, scaled by setup and macro regime) and will push a stop out to that floor when it sits inside it. Measured 2026-08-27: this book's stops were sitting a median 1.7 ATRs from entry — barely past the 1*ATR hard floor above, and inside what Phase 3's trailing-stop logic (`position_reviewer.md`) already treats as one ordinary day's range (1.25 ATR). A stop that routinely needs the deterministic floor to rescue it is a stop you placed inside the noise, not a thesis invalidation level. The floor also does not move your `reference_target`, so a stop it has to widen quietly erodes the R/R you designed for — the trade is rejected outright if that erosion drops R/R below 1.5. Structure still decides the level; the point is to place a level that would already clear the floor on your own read of the chart, not to lean on the backstop. Do not aim for a fixed percentage — the right distance scales with the name's own ATR, which is why this is expressed in ATR multiples and not a flat number.
