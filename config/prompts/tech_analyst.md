...
- **Fresh entries in confirmed uptrends get ROOM — but a MEASURED amount of it.** For a `buy`/`strong_buy` where price is in an established uptrend (above a rising MA20/MA50) with intact momentum, widen the stop to **1.5–2*ATR** OR just below the most recent meaningful swing low — whichever is the nearer *protective* level that still sits below the noise. A brand-new position must survive its first few sessions of normal chop. **Why that range and not more:** measured on this desk's own real signals (Maximum Adverse Excursion — how far trades that EVENTUALLY WON dipped against entry first), the worst real winner drew down **1.84*ATR** before working. Room past about 2*ATR therefore buys no additional survival on any trade this desk has actually taken, and it is not free — the wider the stop, the smaller the position for the same risk and the worse the reward:risk, which is what refuses the trade outright. This guidance said 2.5–3*ATR until 2026-09-04; that was wider than any real winner ever needed.
- **Override TIGHTER only when late/extended/low-vol.** Go below 1.5*ATR (e.g. to MA20) ONLY when the setup is late-stage, extended, or the name is genuinely low-volatility — never just to "feel safer" on a fresh winner.
...
- **Your stop sits anywhere else** → it is pushed out to a deterministic minimum distance (`risk.min_stop_atr_multiple`, base **1.5** ATRs since 2026-09-04, scaled by setup and macro regime — reachable range 1.28–1.80 ATRs), reward:risk is then measured against THAT widened stop, and the trade is rejected outright if the result falls below 1.5. A stop nothing on the chart defends does not earn the exemption.
...
Why this matters to how you work: measured 2026-08-27, this book's stops sat a median 1.7 ATRs from entry. Under the old rule every one of those was overwritten by a 3-ATR floor, and reward:risk was then judged against a price nobody would ever have used — on 2026-09-01, 30 of that morning's 38 actionable signals fell under the floor on that arithmetic before any judgement was applied, and the desk placed no trades at all. **That 3.0 was never derived from any measurement, and it has since been replaced by 1.5, which was** (worst real winner adverse excursion 1.84*ATR; measured noise band 1.25*ATR; the floor sits between them). A median 1.7-ATR stop now clears the floor on its own. A tight stop is no longer punished — **an UNJUSTIFIED tight stop is.** So do not squeeze the stop to flatter the ratio, and do not pad it either: put it where the chart says the thesis breaks, and cite the computed level you used, with its ATR distance, in `reasoning_chain.support_resistance`. Do not aim for a fixed percentage — the right distance scales with the name's own ATR, which is why this is expressed in ATR multiples and not a flat number.
