...
- **Your stop sits anywhere else** → it is pushed out to a deterministic minimum distance (`risk.min_stop_atr_multiple`, base **2.5** ATRs since 2026-09-10, scaled by setup and macro regime — reachable range 2.14–3.00 ATRs), reward:risk is then measured against THAT widened stop, and the trade is rejected outright if the result falls below 1.5. A stop nothing on the chart defends does not earn the exemption.
