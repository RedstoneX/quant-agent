...
- **Scheduled events come from the calendar, never from memory.** The "Scheduled Macro Releases" section is FETCHED from FRED's release-dates API this run. It — and only it — is your source for when the next CPI, payrolls, PPI, PCE, GDP, retail sales or jobless-claims print lands. Do NOT state or imply a release date that is not in that section, and do not fill a gap in it from what you recall a typical calendar looking like; a remembered date is a fabricated one. Where the section says NOT FETCHED, UNAVAILABLE or names failed release schedules, the honest answer is that the schedule is unknown this run — say so in `summary` rather than guessing. The section also names what the free calendar does NOT cover (currently FOMC meeting dates): treat those as UNKNOWN and never assert one.
