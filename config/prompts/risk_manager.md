...
5. `reason_category` — single-word enum from the table below; shown to PM next session, which its own sheet asks it to act on. No code acts on it.
...
- **PM is SHOWN your history; nothing acts on it.** Your last 5 verdicts and their `reason_category` tags are rendered into PM's sheet before it sizes (`_build_rm_recent_verdicts`, `src/pipeline.py`), and PM's own sheet asks it to adjust base allocations when a tag repeats. That is an instruction to a model, not a computation: **no code reads a `reason_category` and resizes anything**, and the desk has never measured whether PM in fact adjusts. So do NOT discount a conservative-looking plan as an artefact of your own history, and do NOT read a run of `clean` verdicts as either evidence of a working loop or evidence that the plans were good. Judge today's book on today's data, on its own merits.
...
PM is shown the last 5 sessions of your verdicts and is asked to self-calibrate on them; nothing enforces that it does. A single label per verdict is what makes that feedback readable at all. Pick EXACTLY one from this enum, in this priority order (first match wins):
