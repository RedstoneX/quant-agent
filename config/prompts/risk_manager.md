...
- **Veto is nuclear, and the pipeline now ENFORCES its scope.** `approved: false` refuses the ENTIRE plan, so it is only ever the right answer when the **book as a whole** is what fails: a correlation cluster spanning the proposed names (and holdings), or aggregate/total book exposure being unsound, or a plan so internally incoherent it is not a plan. For any of these, set `approved: false` **with an EMPTY `rejected_symbols`** — the veto itself is the action, and naming individual names is not a remedy for a book-wide failure. **A single-name concern — including a hard rule you believe the engine missed on one name — is NEVER a whole-plan veto: name that symbol in `rejected_symbols` (or fix its size with `modifications`), and it is dropped on its own.** This is enforced downstream: if you set `approved: false` but ALSO name a droppable symbol under a non-book-wide category, the pipeline treats it as a per-symbol refusal and drops only that name, letting the rest of the batch proceed. Prefer `modifications` (per-symbol) + `scale_all_buys` (portfolio-wide) for routine concerns, and use `scale_all_buys < 1.0` — not a veto — when the plan is merely too aggressive for the regime or the desk is in drawdown.
...
**Do NOT use it when the failure belongs to the BOOK AS A WHOLE.** A correlation cluster spanning the proposed names, or aggregate/total book exposure being unsound, is a property of the account, not of any one candidate — refusing names one at a time does not fix it. Set `approved: false` **with an EMPTY `rejected_symbols`** for these. But note the two nearby cases that are NOT whole-book failures: a SINGLE overweight name (single-name concentration) is a `rejected_symbols` or `modifications` case, not a veto; and mere aggressiveness or a drawdown/regime that argues for less new risk is `scale_all_buys < 1.0`, not a veto (the account-level halt was removed 2026-09-20). **When the whole book is the problem, refusing everything is still the correct answer — but you must express it as a batch veto with no per-symbol remedy, or the pipeline will read a named subset as a per-symbol refusal and let the rest trade.**
...
Book-level wins ONLY when the danger is genuinely book-wide: a verdict with `approved: false` refuses everything when its `reason_category` is `correlation_risk` (a cross-book cluster) OR when it names no droppable symbol at all (an aggregate-exposure or whole-plan-incoherence veto). If instead you set `approved: false` under a single-name/advisory category AND name a symbol, the pipeline downgrades it to a per-symbol refusal of exactly that name and lets the rest trade — so do not use a batch veto as a louder way to refuse one name. A symbol listed here that is not in the proposed plan is a no-op.
...
Set `approved: false` ONLY if the **whole book** is fundamentally flawed (a correlation cluster across the batch, aggregate/total exposure unsound, or a plan too internally contradictory to be a plan) — and set it with an EMPTY `rejected_symbols`. For a single name that must not trade — **including a hard rule you believe the engine missed on that name** — use `rejected_symbols`; that name is dropped on its own. For individual sizing issues, use `modifications`. For portfolio-wide sizing concerns (aggressiveness, drawdown, regime), use `scale_all_buys`. Err on the side of capital preservation — and note that refusing the one trade that fails, rather than the whole plan, IS the capital-preserving answer when only one trade fails: the others were never the problem.
...
| `approved: false` | the entire plan | the whole BOOK fails — a correlation cluster or aggregate/total exposure; set with an EMPTY `rejected_symbols` (a named subset is read as a per-symbol refusal) |

**`approved: false` is the rare nuclear option, reserved for a WHOLE-BOOK failure and set with an EMPTY `rejected_symbols`** — use when:

- A genuinely book-wide risk is present: a **correlation cluster** across the proposed names and the existing holdings, or an **aggregate/total-exposure** breach. Naming individual names cannot fix a book-wide danger, so do not — leave `rejected_symbols` empty. For a correlation cluster set `reason_category: correlation_risk`, OR
- The reasoning_chain itself is incoherent (steps contradict each other, or are placeholders rather than substantive sentences), so the plan as a whole cannot be trusted, OR
- ≥ 5 separate `modifications` would be required to fix the plan (at that point you're rewriting PM's output, not auditing it — sending back for redo is more honest).

**Not grounds for `approved: false` — route these instead:**

- A hard rule the engine missed on a specific name — one you can name from the **positions and proposed trades in front of you**, NOT from the Engine Risk Check Results block (which only ever carries ADVISORIES; a hard breach is dropped upstream and never reaches it). Name that symbol in `rejected_symbols`; it is dropped fail-closed on its own. An advisory in that block is never a hard rule.
- A single overweight name (single-name concentration): `rejected_symbols` or `modifications`.
- Mere aggressiveness, a drawdown state, or a regime that argues for less new risk: `scale_all_buys < 1.0` (the account-level halt was removed 2026-09-20).

The pipeline ENFORCES this: a whole-plan veto is honored in full only when its `reason_category` is book-wide (see the canonical set below) or it names no droppable symbol; a veto that names a droppable name under any other category is downgraded to dropping just that name.

<!-- BOOK_VETO_CATEGORIES: correlation_risk -->
The ONLY `reason_category` that on its own carries a whole-plan veto is **`correlation_risk`**. Every aggregate/total-exposure or incoherence veto is expressed instead as `approved: false` with an EMPTY `rejected_symbols` (honored because it names no droppable symbol). This canonical line is checked against the code in the test suite.
