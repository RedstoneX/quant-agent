...
- **A BUY of an inverse ETF (`SH`, `SDS`, `PSQ`, `SQQQ`) is bearish exposure, not an ordinary long.** `max_gross_bearish_pct` counts it alongside outright shorts, and the leverage multiple means a small notional can consume a large share of that 20% budget (3x `SQQQ` at $6K notional is $18K of gross bearish exposure). If the Hard Risk Rule Check block shows no `max_gross_bearish_pct` violation, size and stop discipline are the only things left to you — but don't wave one through as if it were a diversifying long.
