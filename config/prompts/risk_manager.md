...
- **`R/R n/a` on a RANGE setup** — no computable payoff geometry at all. That IS still a real concern and is different from a thin one: an unknown payoff is not a poor payoff. Python already drops such a target unless its catalyst resolves to a real dated news row, in which case it is kept at the smallest expressible size (`min_position_risk_pct`, {{risk.min_position_risk_pct}}% risk) — unknown geometry, not a numeric reward:risk floor. If one reaches you, expect PM to have explained it.
