...
def create_app() -> FastAPI:
    ...
    if _BOARD_DIR.is_dir():
        # The human status board (`scripts/status_board.py`, run on a timer).
        # It lives under `data/`, which is gitignored, so regenerating it can
        # never dirty the production checkout — the deploy-drift problem that
        # cost a day on 2026-08-28 started exactly that way, with a file the
        # box edited that git also tracked. Same read-only static posture as
        # the mounts above; the GET-only middleware still covers it.
        app.mount("/board", StaticFiles(directory=str(_BOARD_DIR), html=True), name="board")
