...
MAX_ARBITRARY_ENTRIES = 149  # 2026-09-20, board item 32 daily-breaker deployment fix: +1 for `src.risk.rules.BREAKER_FULL_DEPLOYMENT_GROSS`, the gross level the daily circuit breaker treats as "fully deployed". Labelled arbitrary rather than blocking the fix on a source that does not exist. 1.0x is the unlevered book; because it enters as min(gross, P) it has NO effect below itself and bites only on a LEVERED book, where 1.0 is the more permissive choice (a 2.0 pivot would halve a levered book's yardstick). The scaling it governs is bounded by the already-ratified fixed rung, so a wrong pivot cannot disable the breaker. Its open question, and the separate underived choice of WHICH ratified level bounds the scaling, are both stated in the ledger. Was 148, the same day, for board item 124 cross-symbol crowd-out fix: +1 for the new `src.data.smart_money_cluster.MAX_CLUSTER_RESERVED_SLOTS` (the reserved-slot bound), recorded honestly as `arbitrary` with its open question stated -- no measurement yet backs it. Was 147, the same day, for the adversary review below: `src.data.smart_money_cluster.MIN_PURCHASE_CLUSTER_INSIDERS` was `sourced` against an SSRN URL that returns HTTP 403 to everyone, including the author -- a citation nobody can open is not a source under this ledger's own rule. Relabelled `arbitrary` with its open question stated; no value changed. Was 146.
