...
from src.data.levels import find_structural_levels, structural_floor, unfilled_gap_edge
...
def _resolve_structural_stop_and_target(
    ...
    # Parity with the live floor rule (2026-09-12): the stop candidate is
    # `structural_floor` — nearest level on the stop side, ignoring any
    # level on the far side of an unfilled gap — so this engine declines
    # the same signals the constructor's `_require_structural_floor` does.
    close = bars_through_signal[-1].close
    stop = structural_floor(
        all_levels, close, direction,
        gap_edge=unfilled_gap_edge(bars_through_signal, direction),
    )
    if stop is None:
        return None, None, [], {}
    if direction == "long":
