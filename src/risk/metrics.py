...
is hit: `qty x max(0, entry - stop)`. This is the number the owner-ratified 25%
...
once: `qty x max(0, current_price - stop)`. This is audit §1.3's "portfolio
...
def r_multiple(
    ...
    any input is missing/non-finite or when the initial stop was not below
    entry — a non-positive denominator means no risk was defined at entry and
    an R-multiple would be a fabrication.
    """
    ...
    if cur is None or ent is None or stop is None:
        return None
    risk_per_share = ent - stop
    if risk_per_share <= 0:
        return None
    return round((cur - ent) / risk_per_share, 2)
...
def position_risk(
    ...
    """Risk arithmetic for a single long position.
    ...
    if not protected:
        ...
        # as 0.0 would make an unprotected book look like the safest one.
        notional = max(0.0, qty_f * cur_f)
        return PositionRisk(
            ...
            r_multiple=r_multiple(cur_f, entry_f, init_f) if init_f else None,
        )

    assert stop_f is not None  # narrowed by `protected`
    released = stop_f >= entry_f > 0
    budget = 0.0 if released else max(0.0, qty_f * (entry_f - stop_f))
    open_r = max(0.0, qty_f * (cur_f - stop_f))
    return PositionRisk(
        symbol=symbol, qty=qty_f, entry=entry_f, current_price=cur_f,
