...
class Database:
    ...
    # --- Conviction-bar STAY confirmation (owner mandate 2026-09-25) ------
    #
    # One boolean per HELD, reviewed symbol per session: did THIS review find
    # the name below the conviction bar (candidate_eligibility R7)? Persisted so
    # the NEXT review can require TWO CONSECUTIVE misses before culling, the same
    # anti-whipsaw discipline the sourced two-consecutive-close break rule uses
    # (src/risk/exit_guard.py). On `specialist_evidence` like every other
    # cross-session per-symbol flag here — no new table. Keyed on the review's
    # own `bar_date` (the trading day) so several intraday runs of one day are
    # never miscounted as two consecutive reviews, exactly as
    # HOLDING_PROTECTION_BREAK_KIND is.
    CONVICTION_BAR_MISS_KIND = "conviction_bar_miss"

    def save_conviction_bar_miss(
        self, *, run_id: str, symbol: str, missed: bool, bar_date: str,
    ) -> int:
        """Record whether the review dated `bar_date` found held `symbol` below
        the conviction bar. `missed=False` records a CLEAR, which resets the
        streak; only a name recorded missed on two consecutive reviews is a cull
        candidate."""
        return self.insert_specialist_evidence(
            run_id=run_id, agent_name="risk_manager",
            kind=self.CONVICTION_BAR_MISS_KIND, scope="symbol",
            symbol=symbol.upper(),
            evidence_json=json.dumps(
                {"missed": bool(missed), "bar_date": str(bar_date)}
            ),
        )

    def get_prior_conviction_bar_miss(
        self, symbols, *, today_bar_date: str, exclude_run_id: str | None = None,
    ) -> dict[str, bool]:
        """The most recent `missed` flag per symbol from a review dated STRICTLY
        BEFORE `today_bar_date` — the immediately-prior review's verdict.

        A symbol absent from the result has no qualifying prior review on record
        (first look, a gap, or a write that failed) — callers must treat that as
        `False` (no confirmed prior miss), so a missing row can never manufacture
        a two-review confirmation and sell a name early."""
        wanted = [str(s).strip().upper() for s in symbols if str(s).strip()]
        if not wanted:
            return {}
        placeholders = ",".join("?" for _ in wanted)
        ...
        params: list = [self.CONVICTION_BAR_MISS_KIND, *wanted]
        if exclude_run_id:
            sql += " AND run_id != ?"
            params.append(exclude_run_id)
        sql += " ORDER BY timestamp DESC, id DESC LIMIT 500"
        ...
        for row in rows:
            ...
            if not bar_date or bar_date >= today_bar_date:
                continue
            latest[sym] = bool(payload.get("missed"))
        return latest
