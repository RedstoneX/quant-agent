...
- INVALIDATED 2026-09-15: the fixture stored earnings analyses without the pipeline wrapper (`analysis` key), so the PM prompt showed ZERO earnings evidence to every model (both gpt-5.5 and gemini wrote "no earnings rows available"). Fixed and pinned by `test_earnings_rows_use_the_live_pipeline_wrapper_shape`; the PM test must be re-run.
