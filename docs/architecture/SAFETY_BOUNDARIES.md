...
- AI Risk can widen a positive `stop_loss`; that widening IS re-audited (`src/pipeline.py::_apply_risk_modifications`, 2026-09-03): the edit is refused if it makes reward:risk unmeasurable or rests the stop inside the ATR noise band, using the constructor's own arithmetic. That refuses the *edit*, not the ticket. A computed or missing range ratio is not a ticket refusal — invented numeric reward:risk floors are retired (owner 2026-09-17) — and a breakout is not measured on reward:risk at all. This is a known narrow behavior, not permission to redesign risk during unrelated work.
