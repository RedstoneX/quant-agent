...
- The actual rollout script is `ops/review/qamc-recovery-rollout.sh` on branch `claude/trading-utility-recovery-rollout`, which differs from `main` by exactly three files: that script, `ops/review/README-recovery-rollout.md`, and `tests/test_recovery_rollout_script.py` (verified via `git diff --stat origin/main origin/claude/trading-utility-recovery-rollout`; pushed, not merged — matching how `ops/review/*` was handled for the prior finish-line rollout). It is a derivative of the externally reviewed `ops/review/qamc-finish-line-rollout.sh`: Gate B, the deployment-state machine, convergent rollback and every self-integrity check are structurally unchanged; the baseline/target identity, Gate C's focused suite, and one new content-verification block (the seven fix markers, checked three times: pre-checkout, post-checkout, post-enable) differ, as intended.
- **Baseline-state defect, found by external review and fixed before any rollout ran:** the first draft required Gate A's tree to be fully clean and had convergence discard the local delta on rollback — both correct for the *finish-line* script's baseline (`9c736c1`, genuinely clean) but wrong here, because `775296e1` is never clean: the accepted state is that commit plus exactly one authorized local delta (`config/settings.yaml` `intraday_scan.enabled: false -> true`, per this file's own record below). Uncorrected, Gate A would have refused to start, and any post-deploy failure would have converged production to intraday **disabled**. Fixed: Gate A now verifies the tree is byte-identical to committed baseline except for exactly that one line (read-only), Phase 3 discards the delta as a tracked mutation immediately before checkout, and convergence re-applies and independently re-verifies the same delta before ever reporting converged.
- **Second review round — four more defects, all found and fixed before any rollout ran:** the newline-fix's own comparison read committed content via bash `$(...)`, which strips trailing newlines the real working file always has, so the accepted baseline would have been rejected unconditionally — fixed by reading both sides as real files. A full adversarial pass (self-review plus an independent reviewer with no authorship stake) then found: the same function's block-scanner gave up after one line instead of scanning through it like its sibling editor; its regexes rejected the editor's own correct output when the `enabled:` line carries a trailing comment; `converge()` reset rather than ignored a second interrupting signal, so a second real Ctrl-C/TERM/HUP during the restart/health wait could kill the process with the recursion guard still set and the `on_exit` backstop silently no-op — a root-privileged rollback tool going completely silent about whether production converged (fixed by ignoring those signals for convergence's duration, plus a loud backstop message regardless of cause). Full detail, exact triggers and severities are in `ops/review/README-recovery-rollout.md` on the review branch — not duplicated here. `tests/test_recovery_rollout_script.py` (adapted from the finish-line script's own suite, brought onto the branch): 116 → 150 collected tests, **150/150 passed**, re-run against the exact bytes fetched fresh from GitHub, not a local copy, after every fix.
