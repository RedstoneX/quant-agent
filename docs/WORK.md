...
**119. The economics feed can leave required series un-attempted at the open, and the re-derived fix is only measured mid-morning — STILL OPEN on its second box, filed 2026-09-18.** Re-filed out of PR #435 (closed unmerged). **The attempt half is closed (2026-09-26, with item 187):** the shared wall-clock ceiling is no longer first-come-first-served — each configured series is reserved an equal share of it (`total_fetch_deadline_s / len(CONFIGURED_SERIES)`, derived arithmetic over two numbers that already exist, not a new constant), and a retry, a backoff sleep and a `/fred/series` metadata lookup may only draw on the surplus above the reserves of series that have not been asked yet. Fifteen series each spending their whole share is exactly the ceiling, so every required series is attempted in the worst case the budget allows, not merely usually. A series that still never reaches FRED is recorded under its own reason (`not_attempted_deadline_exhausted`, distinct from `fetch_deadline_exceeded`) and named in the analyst's prompt as NOT ATTEMPTED AT ALL, with no value stale or otherwise standing in for it. **What is deliberately NOT built:** the economist is still paid on a partial set. That is a policy question, not a fetch question, and PR #435's version of it was argued against by name in the owner's closing note ("skip/drop as the product"), so it is left for its own change rather than smuggled in here.

DONE WHEN:
  - [x] every required series demonstrably gets a real attempt inside the existing ceiling, proven against open-like conditions rather than a healthy mid-morning batch — MET 2026-09-26: the guarantee is structural (equal reserve per series, second-class work capped at the surplus) and is pinned by a reproduction test in which EVERY call hangs for its whole allowance, strictly worse than the open; against pre-fix code the same test asks 2 of 15, which is the production shape
  - [ ] a series still missing after the bounded re-ask is named, the economist is not paid on the holes, and a later repair pays only once the set is complete — PARTLY met: the miss is now named, distinctly and un-attemptedly; the paid-on-holes and repair-once-complete halves are untouched
