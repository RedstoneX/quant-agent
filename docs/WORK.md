...
**75. The desk locks in almost none of a run-up: its profit-taking is too loose and a chart breakdown cannot trigger a sale — OPEN, filed 2026-09-14 after an owner question on ORCL.** No take-profit order is ever placed (verified in `submit_order`). Profit is banked only by `compute_trailing_stop` (every session: Type A breakeven at +1R, then structure or chandelier), or a reviewer SELL. **ORCL through the live code, daily bars:** `range`, entry 146.27, SL 137.53, target 159.52. 09-04 SL→146.27; 09-08 peak 170.70 but the chandelier move failed the 2% step; 09-09 SL→149.51; 09-11 low 149.84 missed by $0.33; 09-14 gap to 141.42 — out near −3%, or unfilled (stop-limit). An earlier +10.7% came from an invented trail; do not cite it. **Why:** chandelier 3×ATR ≈ $22 on ORCL; the 2% step blocked the peak day; swing lows need 3 bars to confirm; and a SELL on the breakdown is refused because the exit gate accepts only triggers from outside the price series (item 60). All of `CHANDELIER_ATR_MULTIPLE` 3.0, `MIN_RATCHET_PCT` 2.0, `NOISE_BAND_ATR_MULTIPLE` 1.25 and the 3-bar window are unsourced. **Researched (fetched):** a trailing take profit is mainly a crypto-bot feature, mechanically a trailing stop armed once in profit; Alpaca cannot place one (docs.alpaca.markets/us/docs/orders-at-alpaca). Parabolic SAR tightens after spikes, but its constants are one trader's preference. **Position, not ruled:** no fixed ceiling on breakouts; a sell at a range trade's defended level is readable. **Objection:** ranges are the commonest setup, and a ceiling sells as a range breaks out. **What would settle it — a fix, not a patch:** trail tightness read off the instrument or a cited source; the 2% step justified or removed; and a decision on whether confirmed price structure may trigger an exit. One number changed alone is the patch this item exists to prevent.
