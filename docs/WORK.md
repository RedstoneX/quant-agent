...
**The question, answerable in two parts:** (a) how many bars either side must a bar dominate before it counts as a swing point? (b) how far either side of a level's reported price does that level's zone actually extend?

**Where the current numbers came from:** convention, honestly labelled as such in the code. (a) `TRAILING_PIVOT_WINDOW` = 3 in `src/risk/trailing.py` and `PIVOT_WINDOW` = 5 in `src/data/levels.py`. (b) `CLUSTER_TOLERANCE_PCT` = 1.0 in `src/data/levels.py`, whose comment says only that price respects a zone rather than a number — true, and not a derivation of 1%.

**Already searched and ruled out — do not repeat this.** On 2026-09-13 the pivot-window literature was fetched and read: TA-Lib's `FRACTAL` defaults both arms to 2 with no rationale stated; MetaTrader 5's fractal doc defines five bars and gives no reason; five became standard because it shipped as a MetaTrader 4 default, which is a distribution fact and not a measurement; LuxAlgo's swing reference says outright there is no universally best setting. Every source found was assertion, not measurement, and the one that addressed the question directly said no derivation exists. Also settled and not to be re-derived: the two windows never meet — no module imports both, pinned by `tests/test_pivot_window_independence.py` — and the asymmetry runs one way, a bar dominating 5 bars either side necessarily dominates 3, so the trailing window sees strictly more. Making both 3 or both 5 was rejected: that is picking a number. On the zone width, re-tuning the *match tolerance* against it was ruled out and the tolerance was instead derived from the width itself (`level_zone_halfwidth`), so the two can no longer disagree — that made the pair CONSISTENT and did not make the width RIGHT.

**What would settle it:** a study that measures where price actually turns relative to a prior pivot — the empirical distribution of the reaction distance — from which both the window and the zone half-width fall out as readings rather than choices. A microstructure or technical-analysis paper measuring support-zone thickness would answer (b) directly. The strongest outcome is a reformulation: define the zone off the instrument (the pivots' own dispersion within a cluster, or an ATR-scaled band) so no percentage is stated at all.

**Cost while unanswered:** 1% is the desk's definition of "the same level" everywhere, and since 2026-09-13 it is also the slack on whether a stop counts as level-backed — which decides whether the ATR floor moves that stop, which changes reward:risk and therefore position size on every risk-sized trade. On a $200 stock the zone is $2 wide; two bounces $1.90 apart are one level and two bounces $2.10 apart are two, and nothing says that is where the line belongs. The windows cost less because they never meet, but 3 and 5 both already disagree with the archetype's own default of 2.

**56. Is a stop too wide? The width threshold is still unread — but the "three published rules disagree" premise was FALSIFIED on 2026-09-13, and the number doing two jobs has been split. OPEN, narrowed, refiled 2026-09-13.** *Covers the 2.5x ATR fallback stop AND the reach cap that gates it — the same question, asked once.*

**The question, now precisely one thing:** at what probability of being touched inside the trade's own horizon does a stop stop being a stop? Everything else in this item is answered.

**Answered 2026-09-13 (a) — the premise was wrong.** The item was filed on the belief that Kullamägi's "not wider than one ATR" and the desk's 2.5x ATR fallback are rival claims about the same quantity and cannot both be right. They are not rivals: they are stated over different holding periods, and an ATR multiple means nothing until the horizon is fixed. Converted to the one quantity both are really about — the probability the stop is touched before the trade ends — they very nearly coincide. Under the reflection principle for the running maximum of Brownian motion (`P(sup X_s >= a) = 2 P(X_t >= a)`, https://almostsuremath.com/2023/04/18/the-maximum-of-brownian-motion-and-the-reflection-principle/) and the range-to-sigma identity `E[range] = sqrt(8/pi) * sigma` that Parkinson (1980) builds his estimator on (https://portfoliooptimizer.io/blog/range-based-volatility-estimators-overview-and-examples-of-usage/): Kullamägi's 1x ATR cap over the 3-5 sessions his own text gives before the stop moves to break-even ("You should sell 1/3 to 1/2 of the position after 3-5 days, and then move the stop to break even", https://qullamaggie.com/my-3-timeless-setups-that-have-made-me-tens-of-millions/) is a 35.7%-to-47.5% touch probability; the desk's 2.5x ATR fallback over a 20-session horizon is **37.2%**. Same posture, different arithmetic. The claim in the previous draft of this item — "if Kullamägi is right the desk is placing stops roughly 2.5x too wide" — compared a width at one horizon to a cap at another and is withdrawn. Note the comparison is a cap against a width, which is the looser direction, so this reconciles the two rules; it does not derive either.

**Answered 2026-09-13 (b) — one number was doing two jobs, and now two numbers do two jobs.** `MAX_REACH_ATR_MULTIPLE = 1.5` in `src/data/levels.py` was written to estimate how far price could travel TOWARD A TARGET, and since item 54 the stop-width gate refused trades against the same setting — read at the refusal site under the name `max_target_reach_atr_multiple`, with "target" still in the name. Split today into `max_target_reach_atr_multiple` (estimation) and `max_stop_width_reach_atr_multiple` (refusal), same value 1.5, so behaviour is unchanged and the two can no longer be moved by accident together. **Neither is derived. The split is hygiene, not an answer.**

**Answered 2026-09-13 (c) — what the gate is worth, exactly.** The cap `1.5 x ATR x sqrt(H)` scales with sqrt(H) exactly as the reading does, so it refuses at the SAME probability at every horizon: **1.67%**. The gate refuses a stop only when that stop has under a 2% chance of ever being touched inside the trade. It is therefore not blocking good trades — it is **letting wide stops through**, and it is arithmetically incapable of firing on the desk's own fallback: 2.5 <= 1.5*sqrt(H) for every H >= 3, so a band-placed stop can never trip it at any horizon the desk trades. Only a level-backed or signal-bar stop wider than 6.7x ATR (at H=20) or 11.6x ATR (at H=60) is refused. That is consistent with, and explains, the 4-of-101-names measurement recorded on 2026-09-12. Pinned by `tests/test_stop_width_gate.py::TestStopWidthReadingAndSeparation`.

**Also shipped 2026-09-13:** `src/data/levels.py::touch_probability` — the width-to-probability reading above, derived from two published results with no chosen constant, recorded on every stop the constructor sizes (passing or refused) so the desk accumulates the measurement that would settle the remaining question instead of an ATR multiple whose meaning changes with the horizon.

**Already searched and ruled out — do not repeat this.** Kullamägi's 1x ATR/ADR rule: fetched, reconciled above, and it is a cap on an intraday-timed entry's stop, not a multi-day survival measurement. Chandelier and Van Tharp at 2-3x ATR: trailing-stop conventions, magnitude ranges, no derivation. Van Tharp sizing: arithmetic, and prescribes a smaller position rather than a refusal. **Maximum Adverse Excursion (Sweeney) — the technique that appears to answer this exactly — is RULED OUT here, by name, and this is the important one.** MAE is the archetype for "how far do winners go against you before they work", and its own literature states the method as a fit to a sample: "Plot MAE for winners and losers separately. If most winners stayed within some excursion while losers exceeded it, a stop near that boundary cuts losses cheaply", with the caution attached in the same source — "excursion statistics are descriptive of one historical sample; rules tuned tightly to past percentiles inherit all the fragility of any other fitted parameter" (https://www.luxalgo.com/library/concept/mae-mfe-distributions/). That is fitting, which this desk rejects, and it is unavailable regardless: the record is one live trade and 53 archived rows. Ruled out on the same grounds: the trend-following optimisation that reports average P&L maximal near a 4x multiplier (arXiv 2003.09298) — the PDF would not parse and the figure is unverified, but the method is a parameter sweep over historical P&L either way. Also already ruled out, do not re-propose: "no floor, no trade", falsified 2026-09-12 against George & Hwang (2004) and Bulkowski.

**The ACADEMIC literature was searched too, on 2026-09-13, and it says the threshold is unsolved — ruled out by name, do not repeat.** The 2026-09-12 pass had read vendor and trading-educator material only; the finance journals were searched separately after the item 55 research showed that gap. Three findings, all sourced. (1) Han, Zhou & Zhu, "Taming Momentum Crashes: A Simple Stop-Loss Strategy" (https://www.cicfconf.org/sites/default/files/paper_811.pdf) is the most-cited empirical stop-loss study and its threshold is a FLAT 10% with 5% and 15% run as robustness checks — not scaled to the instrument at all, and chosen by sweep. Its own closing sentence on the question: *"we leave the search for optimal stop-loss strategies as future research."* (2) Kaminski & Lo, "When do stop-loss rules stop losses?" (J. Financial Markets, 2014) is the one general framework, and an independent academic review of it states the limitation directly: *"Kaminski and Lo (2007), although interesting, give little guidance for the trader who plan to use stop-loss rules to stop losses that occur over a week, a day or a shorter time period"* (Erdestam & Stangenberg, Stockholm School of Economics, http://arc.hhs.se/download.aspx?MediumId=593). The same review explains the thinness: *"This view may explain the rather limited academic literature on trading rules such as stop-loss."* (3) That thesis itself sweeps its own stop level over a grid, *"γ = 0.5-5%"*, which is the same flat-percentage parameter sweep. So the academic position is: stop-loss rules are studied, the THRESHOLD is exogenous in every one of them, and the leading paper says choosing it is open work. Adopting any of their numbers would be importing a foreign default — the same unsourced act in the other direction, and the exact move item 55 declined on 2026-09-13.

**What would settle it:** a published measurement of stop survival stated as a probability rather than a multiple — the touch probability past which a wider stop stops buying survival and only buys a smaller position. The reading is now built, so the shape of the answer is fixed and only the threshold is missing. Two candidate reformulations that would need no threshold at all and should be tried before hunting further: (i) refuse when the stop's touch probability is lower than the TARGET's reach probability computed the same way on the same instrument — a probability-form reward:risk test, which would replace the arithmetic ratio rather than sit beside it; (ii) accept that no threshold exists and delete the width gate outright, since the ratified §2.1 sizing rule already answers a wide stop with a smaller position and the gate as it stands refuses at 1.67%. Deleting it is a permitted answer and is NOT the lazy hack — the lazy hack would be switching it off without the reconciliation and the reading above.

**Cost while unanswered:** small and now quantified, which is the change from the previous draft. The gate cannot fire on the desk's own fallback stop and fires only past 6.7x ATR on a three-week trade, so it is close to decoration and the direction of its error is known: it under-refuses. Nothing is being wrongly blocked. What remains uncosted is whether anything SHOULD be blocked.
