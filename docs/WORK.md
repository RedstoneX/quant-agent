...
**73. Audit 2026-09-11 leftovers that were never filed — OPEN, filed 2026-09-14.** A four-way read-only audit ran 2026-09-11; its other findings shipped (items 44-46 and the doc/prompt corrections). These three existed only in that session's scratchpad.
(a) **A short's lost stop is detected but never repaired.** `_repair_stop_coverage` rebuilds a stop from the latest recorded BUY row (`src/storage/db.py`, query on `action = 'BUY'`), so a held short's coverage gap is flagged and left naked. The justifying comment in `src/pipeline.py` (~line 2825, "no order path can open a short yet") is false: SHORT is a live opening action (`_POSITION_OPEN_ACTIONS` in `src/storage/db.py`). Re-verified on main 2026-09-14.
(b) **One hard trigger can trim the same symbol twice in a day.** `src/pipeline.py` (~line 9452) self-discloses a RESIDUAL GAP: a symbol trimmed at midday on a hard trigger can be trimmed again at close on the SAME trigger — the 2026-05-04 AMZN shape with a valid phrase. Needs per-event dedup, not a phrase gate. The position-reviewer prompt describes the protection without that caveat. Re-verified on main 2026-09-14.
(c) **~20 trade-governing numbers with no citation, measurement or owner date — audit-reported, NOT individually re-verified; some may have been closed by later items, check each first.** `CLUSTER_CORRELATION_THRESHOLD` 0.7 ("traditional cutoff", no source); insider admission floors ($100k / $250k transaction, 2 owners, $5 price, $10M ADV, 20 days); `stop_atr_setup_scale` and `stop_atr_regime_scale` (multiply into every stop distance); `min_target_atr_multiple` and `breakout_projection_atr_multiple` 1.0; nominations 3 per seat / 6 total and `max_external_candidates` 3 (circular); `max_single_short_pct` 10 (self-flagged stale derivation); `max_gross_bearish_pct` 20; `short_gap_risk_multiple` 1.5; intraday scan 3% move / 3h cooldown; `MIN_RATCHET_PCT` 2.0; `CHANDELIER_ATR_MULTIPLE` 3.0 (real convention, uncited). No-arbitrary-numbers principle applies: no replacement values without the owner.
