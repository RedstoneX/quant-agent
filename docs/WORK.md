...
CONVICTION-FIRST ORDERING SHIPPED 2026-09-25 (owner-prioritised, Design (a)): the morning held-book de-lever now cuts the WEAKEST-BY-CONVICTION names first using THIS session's fresh per-seat read, not a stale persisted stance (the prior stale-based attempt was rejected). The fresh `{symbol:{seat:stance}}` evidence registry DecisionStage builds is stashed on the context together with its §9.4 staleness exclusions; a post-decision pass (`_enforce_gross_ceiling_by_conviction`, morning only, after risk, before execution) builds a cut order (`_conviction_cut_order`) from two ALREADY-RATIFIED rankings — the §9.4 YES/NO (`agreement_refuses_trade`, with stale sources ignored) and the desk's own `rank_verdicts` candidate ordering, counted only for the side the book actually carries — and cuts the lowest first, so a conviction-dead winner is sold before an intact-thesis loser. It does NOT rank by the signed source score as a GRADE: that graded use was retired (board item 66, owner 2026-09-14) and ordering a cut by it would resurrect the retired defect. The pass DELEGATES to `_enforce_gross_ceiling`, which stays the single caller of `apply_gross_ceiling` with trims enabled (`test_trimming_the_held_book_has_exactly_one_owner`). The preamble is scoped on the morning lane to a live-price MARGIN FLOOR — it fires only when the book is levered past the base cap (a genuine forced-liquidation-proximity breach) — but DEFERRAL IS NOT A WAIVER: it records `ctx.gross_ceiling_deferred`, and the morning body's `finally` discharges it with a FULL ordinary-ceiling pass on every lane the conviction pass never reached (the PM-less early returns, the resume lane, an exception exit), so no lane loses the ordinary ceiling. ASYNC-FILL RACE: `wait_for_order_terminal` has a ceiling and can return a non-terminal status, so a trim still working leaves the refreshed book overstating exposure; `ctx.delever_unsettled` then blocks every later de-lever pass this run rather than shedding the same exposure twice. Midday/close/intraday are unchanged: full-ceiling, biggest-loser ordering. The ladder's never-full-liquidation floor is untouched and the ceiling is cleared either way; the total shed is NOT claimed identical — on unequal position sizes the two orderings differ by the pre-existing 1.0% minimum-trim clamp applied to differently-sized names (measured, `test_conviction_order_clears_the_ceiling_and_never_empties_the_book`). The over-ceiling record and the alert decision above are unchanged.
