...
CODE FIXED (PR pending review): BOTH de-lever paths — `_enforce_gross_ceiling` (the gross-exposure ladder) and its sibling `_force_delever` (the `allow_margin=False` cash-deficit sweep) — now price the trim 3% through the market (SELL `* 0.97`, COVER `* 1.03`), reusing the desk's own ratified must-fill-exit buffer `AlpacaBroker.STOP_LIMIT_BUFFER_PCT` rather than a fresh number — the same "prioritize fill over price" buffer the protective-stop leg already accepts. Fixing both keeps the two emergency de-lever paths consistent. This triples the fill probability over the old 1% while, like the stop leg, keeping a LIMIT (capping worst-case fill) instead of a market order (unbounded gap slippage). A gap wider than 3% still misses; for the gross-ceiling path that residual is reported by `_alert_owner_delever_incomplete` and the item-112 shortfall record. Danger-case tests in `tests/test_gross_exposure_ladder.py` prove each path fills on a gap where the old 1% limit would have rested (gross-ceiling SELL and COVER, and the `_force_delever` SELL), and that the gross-ceiling trim takes the correct qty/side without over-trimming below the ceiling.

STILL OPEN (owner risk-appetite decision, not a code question):
  - [ ] whether the emergency de-lever specifically should use a MARKET order (guaranteed fill, unbounded slippage on a fast gap) instead of the 3% marketable limit (may miss on a >3% gap) — the broker already supports market orders (`limit_price=None`); this is the owner's guaranteed-fill-vs-price-protection call, deliberately left as a limit to stay consistent with the ratified stop-limit doctrine
