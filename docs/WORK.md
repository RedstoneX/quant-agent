...
**52. What, if anything, should gate an insider trade on its SIZE — RESOLVED 2026-09-25: size gates nothing.** No published study supports single-transaction dollar size as a positive insider-buy predictor (the closest, Cziraki & Gider 2019, finds size inversely related), so the two invented flat dollar cutoffs (`min_transaction_value_usd`, `external_min_transaction_value_usd`) are deleted from `SECForm4Provider`/`SmartMoneyConfig`; relative size continues to be reported on every observation since 2026-09-13. **Ruled out, with sources: `docs/INCIDENT_HISTORY.md`, 2026-09-13**, including a per-transaction holdings-ratio cutoff that was built and REMOVED — do not reintroduce one without a per-transaction source.
