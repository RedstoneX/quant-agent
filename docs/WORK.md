...
**163. The PM's narrative and its own emitted number disagree with nothing checking it — filed and verified 2026-09-19 against the stored reasoning and target rows.** Run `601011e0` (09-16): `sizing_logic` prose says "RSG and AAPL get 2.5% risk each," but RSG's own emitted `risk_allocation_pct` is 0.5. **CORRECTED 2026-09-26 — the framing above is wrong and is kept only because it is how the item was filed: this is not the seat disagreeing with itself.** The seat asked for 2.5 and a deterministic sub-floor cap rewrote it to 0.5 after the narrative was written; see the re-examination below.
...
DONE WHEN:
  ...
  - [x] the RSG case is re-examined to see which value the seat meant — RESOLVED 2026-09-26, and the answer overturns how this item was filed: **the seat never emitted 0.5 and never contradicted itself.** Stored run `run-601011e0` (2026-09-16) read back read-only from the production database. The portfolio manager's own raw response (`agent_logs.full_response`, the text the model returned) contains the string `0.5` NOWHERE: it asked for AAPL 2.5 / RSG 2.5 / NET 2.25 / TSM 2.2 / MRVL 2.0 / ZS 1.75, exactly the ladder its `sizing_logic` narrates. The 0.5 that reached the `portfolio_manager` / `target` evidence rows for RSG and ZS was written AFTER parsing by the desk's own deterministic sub-floor size cap — `PortfolioManagerAgent._apply_subfloor_catalyst_rule` as it stood at commit `0d091ae7` (the code live that session), which capped an increase to `starter_risk_pct` (0.5) when a range setup's reward:risk was under the reference; RSG was a RANGE setup at R/R 0.81 (entry 223.99, stop 218.51, target 228.45, and the risk seat's own `rr_audit` says so). That cap's log line already calls it "Deterministic, not PM inconsistency". RSG was then traded at the capped size (`proposed_order`, BUY, 20.44% allocation). So the 2.5% is what the seat meant AND the 0.5% is the correct emitted size — both are right, and the defect is the NARRATION LAG: `sizing_logic` is written before the mechanical adjustment runs and is never restated, so the owner-facing story describes a trade that was not taken. This is the same ordering problem `PortfolioDecision.constructor_dropped` exists to explain to the Risk Manager, on the sizing field instead of the symbol list. Diagnosis only; nothing about the cap or the stored run was changed.
  - [x] the mismatch is recorded and surfaced, NOT blocked — decided 2026-09-26 on the reproduction above, not on principle. The tempting rule ("a size the seat's own reasoning contradicts is fabricated, refuse the name") would, on the one case the desk has actually measured, have refused a correctly-capped RSG buy over a stale sentence. Telling "the seat contradicted itself" from "a deterministic rule moved the number after the seat wrote about it" is what a blocking gate would need and what nothing currently has. The detector therefore stays detection-only: durable per-symbol evidence rows (`sizing_narrative_check` / `mismatch`, carrying both numbers), no target refused, no number corrected.
  - [x] the tolerance is read from the emitted field, not chosen — 2026-09-26. The check previously borrowed `RISK_NARRATIVE_MISMATCH_TOLERANCE_PCT` (0.5pp, one `min_position_risk_pct` increment). That is a risk-BUDGET floor, not a statement about how precisely `risk_allocation_pct` is written, and at 0.5pp it called prose "2.5% risk" and an emitted 2.0 the same risk — a 25% sizing difference passing unseen. It now compares at HALF THE LAST PLACE OF THE FIELD AS EMITTED: a field written `2.0` asserts a tenth, so [1.95, 2.05] is the same number said differently and 1.9 is not; a field written `2.25` asserts a hundredth and gets a ±0.005 band. Nothing is chosen — both bands fall out of the digits the field itself carries. The per-symbol `TargetPosition.thesis` check keeps its own constant and is untouched.
...
**185. Trailing-stop numbers are made-up money numbers with no board item — filed 2026-09-25.** Item 90's half two, surfaced for visibility. How much of a run-up the desk gives back is a flat 3x-ATR chandelier (`trailing.CHANDELIER_ATR_MULTIPLE`, called "the conventional setting" with no citation), a proposed stop must ratchet at least 2% above the live stop (`trailing.MIN_RATCHET_PCT`), and midday refuses a trail whose new stop sits below 50% of the current price as a likely model typo (`_midday_execute_llm_actions`). All `status: arbitrary`. The trailing pivot window is already tracked by item 55 and the range ratchets by item 142, so they are excluded here. **2026-09-26 pass — two of three closed, item STAYS OPEN on the third.** `CHANDELIER_ATR_MULTIPLE` was already `sourced` against the published Chandelier default and is untouched. `MIN_RATCHET_PCT` (2%) is CLOSED as owner-ratified churn appetite — the 2026-09-25 adversary ruling stands, and its open question was rebuilt because the old one was false: it claimed live capital would eventually supply the commission/spread/replace cost that settles the number, but US-listed equity orders at this desk's broker are commission-free and replacing a resting stop crosses no spread and prints no fill, so that cost is ~zero and no future measurement can derive 2.0; the real cost of churn is operational (rate limits, the brief unprotected replace window, alert noise). The only question left there is redundancy, answerable from the desk's own refusal logs with no new number. The midday 50%-of-price guard is NOT closed: it is a sanity bound on model output, not a trading number, but it stays counted because the universe screen derives a live volatility ceiling from it. Its old question — the widest stop the desk's own rules can legitimately produce — is now ANSWERED from the repo at 3.00 x ATR14 (base 2.5 scaled by breakout 1.00 and risk-off 1.20; reachable range [2.1375, 3.00]), so a legitimate stop drops under half of price exactly when ATR14/price > 16.67%. What blocks closure is a circularity: the guard and the screen ceiling each derive from the other, and one of the two has to be chosen independently.

**DEFECT FOUND, NOT FIXED — pre-existing, needs the owner's call because it tightens a live screen.** `universe_screen.STOP_SANITY_FLOOR_FRACTION` divides by `min_stop_atr_multiple` (2.5), giving a 20% ATR/price ceiling, but the widest REACHABLE stop multiple is 3.00. Names with ATR14/price between 16.67% and 20% therefore pass the screen and then have their widest legitimate stop refused by the midday guard as a typo — the screen's own "fails by construction" rationale does not hold over that band. The fix is to divide by the widest reachable multiple, not the base one.
...
DONE WHEN:
  ...
  - [ ] the midday 50%-of-price sanity floor stops being a chosen number. ONE OWNER ANSWER SETTLES IT: the highest ATR14/price a name may carry and still enter the universe. Call it C, and the floor reads 1 - 3.00 x C (C = 16.67% reproduces today's 0.5; C = 20%, today's effective screen ceiling, gives 0.40). Picking either end without C only moves the arbitrary number to the other end
  - [ ] the screen-divisor mismatch above is either fixed or recorded as accepted
...
DONE WHEN:
  ...
  - [ ] ***OWNER APPETITE*** at what pairwise |correlation| should two names count as ONE bet? Lower (0.5-0.6) rations harder and refuses ideas as duplicates; higher (0.8) splits themes and lets a crowded book pass. Clustering here is TRANSITIVE, so a lower cutoff grows clusters faster than the number suggests. The no-number alternative is a build: distance-based hierarchical clustering, which has no threshold to choose
