...
**99. Analyst-seat prompt audit and enforcement gap — TIER 2, filed 2026-09-18; not yet placed in the owner's priority order. (d) SHIPPED 2026-09-26; (a), (b), (c), (f), (g) still open. Detail: `docs/BOARD_NOTES.md` ("item 99").** **Do not build a fourth prompt-drift grep.** There are three and they do not overlap: `retired:` + `scan()` finds surviving prose for a DELETED mechanism, `described:` + `described_gaps()` fails the build AT the deletion so the first one gets written, and `src/prompt_bindings.py` (item 107) catches behaviour that CHANGED while still existing.

DONE WHEN:
  - [ ] (a) the ~55 numbers that exist only as prompt prose and the ~20 unsourced market-structure claims are each sourced, rendered from the code value, or deleted. Part of this set is already catalogued precisely under item 107's own DONE WHEN (the PM sheet's sizing arithmetic and the technical sheet's freshness and valuation tiers, each verified 2026-09-26 as computed by no code and absent from the ledger) — resolve them there rather than re-listing them here. The remainder has never been enumerated one by one.
  ...
  - [x] (d) DONE 2026-09-26 — the deletion-site check is now a TRIGGER, not a convention. `config/retired_mechanisms.yaml` gained a `described:` section pinning live code symbols to the exact places prompt text describes them; `described_gaps()` fails the build the moment one is deleted or renamed, names every sentence that just became untrue, and points at the `retired:` entry that must be written — which is what makes the existing phrase scan fire. Four mechanisms registered; all four anchor Python-assembled agent strings and three of them anchor no markdown at all, because both confirmed drift cases on this desk were run-time strings, not prompt files. A blanket symbol scan was measured and REJECTED: 430 snake_case tokens in `config/prompts/*.md`, 151 resolving to no Python definition because they are enum literals and JSON fields the seats emit — 151 false positives on day one.
  - [ ] (f) "2+ oversized → cut every BUY 25%" stops being unenforced prose — enforced, sourced or deleted, same class as (a). RE-VERIFIED 2026-09-26 against `origin/main` and it is worse than "unenforced": `config/prompts/portfolio_manager.md:643` instructs the cut, and `config/prompts/risk_manager.md:41` asserts to a SECOND paid seat that the loop already runs ("2+ `oversized` tags cut its base allocations 25%"), telling the risk manager to discount a conservative plan as anchoring rather than conviction. No code applies it; nothing in `src/` reads a count of `oversized` tags to scale anything. The 25% is not in `config/number_ledger.yaml`. NOT resolved here and no value picked: the choices are enforce it, source it, or delete it, and deleting a sizing instruction from the sheet that sizes trades is a money decision, not a documentation pass.
