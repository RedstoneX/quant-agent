...
CODE FIXED (PR pending review): the trim is now priced 3% through the market (SELL `* 0.97`, COVER `* 1.03`), reusing the desk's own ratified must-fill-exit buffer `AlpacaBroker.STOP_LIMIT_BUFFER_PCT` rather than a fresh number — the same "prioritize fill over price" buffer the protective-stop leg already accepts. This triples the fill probability over the old 1% while, like the stop leg, keeping a LIMIT (capping worst-case fill) instead of a market order (unbounded gap slippage). A gap wider than 3% still misses; that residual is reported by `_alert_owner_delever_incomplete` and the item-112 shortfall record. Danger-case tests in `tests/test_gross_exposure_ladder.py` prove the trim fills on a gap where the old 1% limit would have rested (SELL and COVER), and that it trims the correct qty/side without over-trimming below the ceiling.
