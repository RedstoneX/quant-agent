...
**60 — one of the four gates is measured and one hole in it is CLOSED (2026-09-14). The owner call above stays OPEN.** The ATR noise band's only non-redundant domain is thesis-invalidation exits, and on exactly that class the desk's own `check_structural_protection` read was never consulted — fixed, additively, with no exit blocked today becoming allowed. The band has never actually altered an outcome. Mechanism, method, and what was deliberately not done: `docs/INCIDENT_HISTORY.md`, 2026-09-14. **Still open:** the owner call above, and the band's own multiple (item 70).
