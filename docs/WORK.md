...
**25. "Don't sell a protected position without a named reason" is prompt-only, same shape as the PM's catalyst gap — PARTIALLY ADDRESSED, needs sign-off.** `risk_manager.md` asks the RM to confirm a sell trigger itself; no Python verified it against real data. Same "citation exists, substance unchecked" shape as item 18's catalyst-door finding. 2 of 3 allowed exits are now checked (`exit_guard.py::holding_discipline_false_claim`): regime-flip claim vs macro; bearish state_change claim vs news. The flat "<5 days" protection window itself has now been REPLACED (2026-09-03/04, `exit_guard.py::check_structural_protection`): protection is decided by whether the trade's `thesis_invalid_if` condition (via `check_thesis_invalid_if`) or, absent one, the structural level backing its stop (same level/touch/ATR-identification machinery `portfolio_constructor.py::_level_backing_stop` uses) has actually broken — no day count anywhere in the decision. A break only counts on the CLOSE (never an intraday quote), by at least `NOISE_BAND_ATR_MULTIPLE` beyond the level, and only once confirmed on TWO CONSECUTIVE trading-day closes (a one-day wick/"spring" reclaim does not lift protection). A position with neither a stated condition nor a qualifying level falls back to the existing noise band (`adverse_move_is_noise`) rather than losing protection outright — breakout/momentum trades without classic multi-touch structure are not systematically stripped of it. Precondition fixed 2026-09-03: `thesis_invalid_if` used to reach `TradeDecision` only as text two truncations (500/280 chars) could cut off; now a dedicated, untruncated field + `trades` column — see `docs/INCIDENT_HISTORY.md`. Still open, unrelated to this piece: whether a proven-false regime/state-change claim should ever escalate beyond logging to a veto. Detail: `INCIDENT_HISTORY.md`, "item 25."
