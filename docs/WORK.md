...
**60 — one of the four gates is measured and one hole in it is CLOSED (2026-09-14). The owner call above stays OPEN.** Two facts about the deterministic gates were established by running them, not by reading them. (1) **The ATR noise band's entire non-redundant domain is thesis invalidation.** Each of the 26 `_HARD_TRIGGER_KEYWORDS` was run through `src.risk.exit_guard.cites_external_information`: 21 of 26 match it and therefore bypass the band outright. The five that do not are `thesis_invalid`, `thesis invalid`, `invalidation triggered`, `broken thesis`, `thesis broken`. Any admissible exit reason that is not a thesis-invalidation claim skips the band automatically, so the band adjudicates that one class and nothing else. (2) **On that one class, `check_structural_protection` was never consulted.** `TradingPipeline._holding_discipline_check_for_exit` returned early unless the reason claimed a regime flip or a bearish state change; a thesis-invalidation reason claims neither, so the read never ran — on the exit class where "has the level backing this stop closed beyond it on two consecutive sessions?" is the entire question, and the desk already computes that answer. The band answered a different question in a different unit ("is this move bigger than one ATR?") and was the only thing standing there. **Fixed:** the short-circuit now also admits a thesis-invalidation claim (`exit_guard.claims_thesis_invalidation`, built from the same five keywords), the structural read runs, and its verdict is filed as append-only per-symbol evidence (`kind="thesis_invalidation_structural_check"`) and logged. **No exit that is blocked today becomes allowed** — additive by construction: the read is taken with `persist=False` so it can never become the prior-day half of a future confirmation (filing it would let a break confirm a session early, lift `protected` a session early, and release a currently-blocked exit by side effect); the verdict is not fed to `holding_discipline_claim_check`, which still leaves (a) `thesis_invalid_if` unjudged; and the method still returns None on an (a)-only reason. Tightening the sell path when the level comes back INTACT was considered and deliberately not done — a ratifiable decision, not a side effect of wiring up a check. **Also measured and unchanged:** the band has never altered an outcome — all eight blocked discretionary exits in the archive independently fail the named-trigger gate too, so "the band is too tight" is not an argument any evidence here supports. **Still open:** the owner call above, and the band's own multiple (item 67).
