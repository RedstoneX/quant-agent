...
**80. Stop provenance — every shipped stop must trace to a computed level, the signal bar, or the volatility band. RESOLVED 2026-09-25 (owner ruling).** A model-typed stop matching no level, signal bar or volatility band used to be honoured, letting an unverifiable number size a real trade. The contested REFUSAL path was overruled: a missing ATR is never a reason to skip protection. When there is no ATR the stop is now DERIVED from price structure (nearest verified computed level on the protective side, else the signal/prior bar) and the position is HELD; the name is skipped only when no structural level is readable at all, or the only readable one implies risk past the desk's stop-distance sanity bound. Buffer past the level is owner-appetite, ledgered.
