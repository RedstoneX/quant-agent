...
**54. "No floor, no trade. No ceiling is fine" — DECIDED 2026-09-12 by the owner, IN FLIGHT (PR #330 open, not merged).** What the code did before: nothing required a floor. The constructor refused only when the chart yielded NO levels at all, or when neither the PM nor the analyst had typed a stop; a long with six resistances overhead and nothing beneath shipped, its unbacked stop pushed out to the ATR noise band — a distance, not a level. The backtest engine already declined the same trade, so the live desk and its own rehearsal disagreed. Now: a trade with no computed structural level on the STOP side of its entry (below a long, above a short) is refused by name — `no_structural_floor` — on BOTH setup types, at the PM-eligibility preview (rule R6) and at order construction, from the one funnel both share. The refusal is recorded as data (`constructor_refused` event with the code beside it, drained from the constructor's own structured record, never the log-text regex) and the census and the PM-facing digest read it as its own bucket. Nothing OVERHEAD is required: a breakout at new highs has no ceiling by definition, its target is the ATR measured move, and the corresponding refusal stays unreachable. No timer, no bar count, no gap-vs-pop filter: the levels are re-read every session and a name qualifies the day the chart shows a floor. **Prerequisite shipped in the same PR — the level scan's relevance window is now READ from the instrument.** It was a flat 40% of price with no derivation; it is now `horizon_reach` — the SAME `ATR x sqrt(sessions) x 1.5` the target derivation already uses to decide whether a level is reachable — at the 60-session horizon cap the desk already imposes, i.e. the furthest any permitted trade could travel (~11.6 ATR). No new constant. It widens on a volatile name and narrows on a quiet one, which matters now that "no floor" refuses: a floor the stock can reach is never dropped for sitting past an arbitrary line. Not a data fault: "could not compute levels" (short/dirty history) is PR #326's split and is refused earlier by its own name. **Second ruling, same day, after the first build was measured:** on a $50 -> $80 gap from an established base the gap itself inflates the measured ATR, so on the gap day the pre-gap $50 shelf was still inside reach and the code took it as the floor — 37% below, a tiny trade. The owner: *"A shelf $30 below, on the far side of a gap the market has repriced through, isn't support anyone is defending."* So **a structural level on the far side of an UNFILLED gap does not count as a floor** (mirrored for shorts: a down-gap disqualifies a ceiling). Derived from the chart via the desk's ONE existing gap detector (`src/data/context.py::find_unfilled_gaps`, the same gaps the analyst's context block already reports) — no second definition, no timer, no decay: a gap that fills stops disqualifying anything because the condition is re-read each session. Its own refusal code (`floor_only_beyond_unfilled_gap`) so the census can tell it from "no levels at all" and "levels only overhead". Real numbers: gap day, the $50.60 shelf is beyond the unfilled gap and the trade is refused; ~2.5 weeks later a base at $74-80 has bounced twice off $75 and the $74.40 floor above the gap qualifies. **Inherited, flagged, not derived:** the reused detector ignores gaps under 2% (`_MIN_GAP_PCT`), a pre-existing "worth noting" threshold.
