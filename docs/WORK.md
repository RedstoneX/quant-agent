...
**The gate is EMPTY as of 2026-09-14 — every PM-gate item is closed, and the
owner's condition (no model test runs while any gate item is open) is met.**
The last item open, 7, was established not to be a gate at all: a model
comparison hands every candidate model the IDENTICAL frozen input, macro
block included, so the size or shape of that input cannot change which model
wins — it is a one-model prompt question, not a between-model one. Moved to
the main backlog as item 76, 2026-09-14 (still real, open work — just not a
gate on this test).

**A known limit on what the test would measure, not a gate on running it:**
the only real-day scenario, `pm_selection`, runs on a fixture where zero of
59 rows carry `computed_levels`, so the structural reward:risk the desk's
live admission rule cannot be computed for any name on it. Full detail and
the fix already scoped: item 72.
...
**32. Should the desk have ONE drawdown response or two — OWNER CALL, the only thing left on this item.** *Reconciliation half closed 2026-09-14; the daily breaker now halts rather than liquidates (`docs/INCIDENT_HISTORY.md`, 2026-09-14). Everything under the question is context, not work.*
...
**73. A short that loses its protective stop is never given one back, by either repair path — OPEN, filed 2026-09-14 from the 2026-09-11 audit, widened and re-verified on origin/main the same day.** The desk repairs a missing stop on a LONG automatically and leaves a SHORT naked. Four independent defects produce that, and fixing any one alone repairs nothing: (1) the lookup reads only `action = 'BUY'` rows (`src/storage/db.py`), so a short has no row to read; (2) the repair itself is long-only — `src/execution/stop_repair.py` hard-codes `side="sell"` and refuses whenever `stop_price >= price`, and a short's stop always sits ABOVE price; (3) the in-session reconciler (`Pipeline._reconcile_stop_coverage`) flags a short's gap and returns without repairing; (4) the half-hourly coverage sweep built for item 53 (`src/coverage_watchdog.py`) copied the same rule and skips shorts outright. **The stated justification is false on two counts.** The comment in `src/pipeline.py` says "no order path can open a short yet" — but SHORT is a live opening action the constructor builds and `_POSITION_OPEN_ACTIONS` records. And both paths say there is "no recorded entry row" to read a level from — but the SHORT row DOES store one (`stop_loss=stop_price` at the entry write in `src/pipeline_stages.py`). So no policy call is needed: the level is on record exactly as it is for a long. **Why it ranks high:** it can leave a live position unprotected, and the long-side repair gives false comfort that coverage is handled. **Not established:** whether a short is held at the broker right now (the production trades table records none, and no borrow-side gate beyond `get_shortability` was found), and whether any risk rule blocks a short from filling. **What would settle it:** a short-side repair reading the level off the SHORT row, placing a BUY stop, with the side/price guard mirrored rather than bypassed; both repair paths calling it; a test that loses a short's stop and asserts it comes back. No number is chosen.

**74. One piece of news can cut the same holding twice in a day, and whether that is a fault is a design question — OPEN, filed 2026-09-14 from the 2026-09-11 audit, re-scoped the same day.** A symbol reduced at midday on a hard trigger (e.g. "bearish earnings") can be reduced again at the close on the SAME trigger. `src/pipeline.py` discloses this itself as a RESIDUAL GAP (~line 9651) and only logs a warning on the second cut. The position-reviewer prompt's rule 4 does state the exception — a second REDUCE/SELL is allowed when a hard trigger such as bearish earnings fires and is cited — so the prompt is NOT silent about it; an earlier filing said it was, and that claim was wrong. Nothing deduplicates by EVENT, and the claim check that runs on exits tests regime and state-change claims, never earnings. **Why it is a question, not a plain fault:** a close session can legitimately read the same filing as worse than the midday session did, and a per-event dedup would forbid acting on that. The 2026-05-04 AMZN double cut is the harm on the other side. How rare it is, is unmeasured. **What would settle it:** a decision on whether a hard trigger is spent once acted on for a symbol that day, or may be re-read — and if re-read, what distinguishes a worse reading from the same reading applied twice. No number is proposed.

**Unsourced numbers from the 2026-09-11 audit — inventory, not an item.** ~20 trade-governing numbers, re-checked 2026-09-14: all live, three owned by items 52/56/70, the rest by nothing. List: `docs/INCIDENT_HISTORY.md`, 2026-09-14. Each becomes an outcome-3 item when worked; do not re-audit.

**75. The desk has no automatic profit-taking: its target never reaches the broker, a trim for profit is not an allowed exit reason, and the trail sits too loose — OPEN, filed 2026-09-14 after an owner question on ORCL, traced session by session 09-02..09-14.** **Every seat saw the target (production agent_logs, verified 2026-09-14).** Level scan → TA `reference_target` 159.52; the PM used it for R/R and the Risk Manager saw it; the position reviewer was shown "Reference target: $159.52 (soft — you manage exit)" every session. Used only to justify the entry: the reviewer is told it manages the exit, but its sell gate refuses profit-taking. Selling at target: +9.1%. **Owner asked (2026-09-14):** show each target on the Mission Control chart, and debate using it (week-long paper trade argued against: `docs/BOARD_NOTES.md` item 75). **Proposed, not ruled:** shadow-track without orders, rules fixed before looking — sell all at target / half at target + trail / target arms a tighter trail / current desk. **ORCL through the live code** (`range`, entry 146.27, SL 137.53, target 159.52, 1800-day levels): the only automatic exit is the trail filling 09-14 ≈145.02, below entry. The one truthful profit path the code allows: a discretionary reviewer TRAIL_STOP at the 09-08 15:30 close to ≈153.6, filling 09-10 ≈153-154 (+5%) — nothing prompted it and the prompt default for a winner is HOLD. **Blind spots, each as of the day:** `take_profit` is never sent to the broker (target 159.52 traded through 09-04 and 09-08); the target-breach flag needs >150% progress (`pipeline.py`, ORCL peaked at 122%); profit-taking, rejection off a high and upcoming earnings are not allowed SELL reasons (trigger-word gate); only the news seat emits `state_changes`, so a chart breakdown cannot unlock an exit; `thesis_invalid_if` is pinned at entry and never follows new structure; the earnings fetcher reads only 10-Q/10-K, so the 8-K results (09-10 16:16 ET) were invisible until the 10-Q (09-11 16:10 ET); levels feed prompts only. **Unsourced numbers in the trail:** `CHANDELIER_ATR_MULTIPLE` 3.0, `MIN_RATCHET_PCT` 2.0, `NOISE_BAND_ATR_MULTIPLE` 1.25, the 3-bar swing window, the 150% breach flag, and the discretionary 4-day cooldown. Trailing take profit: crypto-bot feature, not placeable at Alpaca (fetched). **What would settle it — a fix, not a patch, and never fitted to ORCL:** exits confirmed by several independent signals (structure break on a close, volume, news/earnings, sector) so one wiggle cannot whipsaw; trail tightness read off the instrument or a cited source; a decision on whether the target and a confirmed breakdown may trigger an exit; and the 8-K gap closed. One number changed alone is the patch this item exists to prevent.

**76. PM-input shape — the one open piece is whether the PM uses its new macro-audit channel — OPEN, moved out of the PM TEST GATE 2026-09-14 (was gate item 7).** The volume/shape redesign itself is DONE; full section-by-section measurement lives in `docs/INCIDENT_HISTORY.md`, 2026-09-13, "item 18d / PM gate item 7" — do not restate the figures here. **What is left:** the PM's output schema now has a `reasoning_chain.macro_audit` field (see item 18) so a genuine macro-logic-error finding has somewhere to go — across 56 archived `portfolio_manager` calls, 27 carried macro's full `reasoning_chain` under an instruction to audit it, and ZERO responses ever used it. That is "no evidence it works", not proof it cannot; the archive ends 2026-09-02 on a modest sample. **What would settle it:** a before/after benchmark run to see whether the seat actually uses the channel. Do NOT re-open this as a size problem — Technical Analysis and Independent Source Agreement are already bounded and scale with candidate count, not per-name shape.
