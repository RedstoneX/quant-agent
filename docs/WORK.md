...
CONVICTION-FIRST ORDERING SHIPPED 2026-09-25 (owner-prioritised, Design (a)): the morning held-book de-lever now cuts the WEAKEST-BY-CONVICTION names first using THIS session's fresh per-seat read, not a stale persisted stance (the prior stale-based attempt was rejected). The fresh `{symbol:{seat:stance}}` evidence registry DecisionStage builds is stashed on the context; a post-decision pass (`_enforce_gross_ceiling_by_conviction`, morning only, after risk, before execution) ranks held names by signed source score and cuts the lowest first, so a conviction-dead winner is sold before an intact-thesis loser. The preamble `_enforce_gross_ceiling` is scoped on the morning lane to a live-price MARGIN FLOOR — it fires only when the book is levered past the base cap (a genuine forced-liquidation-proximity breach), remaining the sole enforcer on every PM-less lane. Midday/close/intraday are unchanged: full-ceiling, biggest-loser ordering. The amount shed and the ladder's never-full-liquidation floor are untouched; only the trim ORDER changes. The over-ceiling record and the alert decision above are unchanged.
