...
DONE WHEN:
  ...
  - [x] the RSG case is re-examined to see which value the seat meant — RESOLVED 2026-09-25: the seat MEANT 2.5% for RSG; the emitted `risk_allocation_pct=0.5` is the erroneous field. Stored run `run-601011e0` (2026-09-16) read back read-only (agreeing across two snapshots): `sizing_logic` states "RSG and AAPL get 2.5% risk each as the best breadth/diversification adds" inside a deliberate conviction-graded ladder (AAPL 2.5 / RSG 2.5 / NET 2.25 / TSM 2.2 / MRVL 2.0 / ZS 1.75), of which every name emitted its stated number EXCEPT RSG (0.5) and ZS (0.5) — both collapsed to the exact risk floor (`min_position_risk_pct`=0.5). RSG's target carried `conviction=high` and was ranked top-equal with AAPL (which did emit 2.5), and portfolio at-risk was 2.72% against a 25% ceiling (22.28% headroom), so no ceiling clamp forced 2.5 down; the 0.5 emission contradicts the stated intent, the high conviction and the AAPL pairing. DIAGNOSIS ONLY — nothing is changed; the 0.5 that was authoritatively used at the time is untouched. The shipped detector reproduces the finding: run read-only through `check_sizing_narrative` it flags RSG (prose 2.5% vs field 0.5%). (Second observation: ZS shows the same prose-vs-field gap, 1.75% stated vs 0.5% emitted, but its prose omits the word "risk" so the conservative matcher correctly stays silent — a MISS by design, not a defect.)
