...
**56. Is a stop too wide? The threshold is still unread — but the "three published rules disagree" premise was FALSIFIED, and the number doing two jobs is split. OPEN, narrowed, refiled 2026-09-13.** *Covers the 2.5x ATR fallback stop AND the reach cap that gates it — one question.*

**The question, now precisely one thing:** at what probability of being touched inside the trade's own horizon does a stop stop being a stop? Everything else here is answered.

**Answered (a) — the premise was wrong.** The item assumed Kullamägi's "not wider than one ATR" and the desk's 2.5x ATR fallback are rival claims about one quantity. They are not: an ATR multiple means nothing until the horizon is fixed. Converted to the quantity both are about — the probability the stop is touched before the trade ends — they nearly coincide. Under the reflection principle for the running maximum of Brownian motion (`P(sup X_s >= a) = 2 P(X_t >= a)`, https://almostsuremath.com/2023/04/18/the-maximum-of-brownian-motion-and-the-reflection-principle/) and the range-to-sigma identity `E[range] = sqrt(8/pi)*sigma` behind Parkinson (1980) (https://portfoliooptimizer.io/blog/range-based-volatility-estimators-overview-and-examples-of-usage/): his 1x ATR cap over the 3-5 sessions his own text gives before the stop moves to break-even (*"sell 1/3 to 1/2 of the position after 3-5 days, and then move the stop to break even"*, https://qullamaggie.com/my-3-timeless-setups-that-have-made-me-tens-of-millions/) is a 35.7-47.5% touch probability; the desk's 2.5x ATR over 20 sessions is **37.2%**. The previous draft's claim — "if Kullamägi is right the desk's stops are 2.5x too wide" — compared a cap at one horizon to a width at another and is WITHDRAWN. It is a cap against a width, the looser direction: this reconciles the two rules, it does not derive either.

**Answered (b) — one number was doing two jobs.** `MAX_REACH_ATR_MULTIPLE = 1.5` (`src/data/levels.py`) estimates travel toward a TARGET; since item 54 the stop-width gate refused trades against the same setting, read at the refusal site under the name `max_target_reach_atr_multiple`. Split into `max_target_reach_atr_multiple` (estimation) and `max_stop_width_reach_atr_multiple` (refusal), same value, so behaviour is unchanged and neither can be moved by accident. **Hygiene, not an answer — neither is derived.**

**Answered (c) — what the gate is worth.** The cap `1.5*ATR*sqrt(H)` scales with sqrt(H) exactly as the reading does, so it refuses at the SAME probability at every horizon: **1.67%**. It refuses a stop only when that stop has under a 2% chance of ever being touched. So it is **letting wide stops through, not blocking good trades**, and it is arithmetically incapable of firing on the desk's own fallback: 2.5 <= 1.5*sqrt(H) for every H >= 3. Only a level-backed or signal-bar stop past 6.7x ATR (H=20) or 11.6x ATR (H=60) is refused — which explains the 4-of-101-names reading of 2026-09-12. Pinned by `tests/test_stop_width_gate.py::TestStopWidthReadingAndSeparation`.

**Also shipped:** `src/data/levels.py::touch_probability`, the width-to-probability reading above — two published results, no chosen constant — recorded on every stop the constructor sizes, so the measurement that would settle this accumulates by itself.

**Already searched and ruled out — do not repeat.** Kullamägi: reconciled above; a cap on an intraday-timed entry, not a multi-day survival measurement. Chandelier and Van Tharp at 2-3x ATR: conventions, magnitude ranges, no derivation. Van Tharp sizing: arithmetic, and answers width with a smaller position, not a refusal. **Maximum Adverse Excursion (Sweeney) is ruled out by name** — it is the archetype that appears to answer this, and its own literature states it as a fit: *"Plot MAE for winners and losers separately. If most winners stayed within some excursion while losers exceeded it, a stop near that boundary cuts losses cheaply"*, with the caution attached in the same source, *"excursion statistics are descriptive of one historical sample; rules tuned tightly to past percentiles inherit all the fragility of any other fitted parameter"* (https://www.luxalgo.com/library/concept/mae-mfe-distributions/). Fitting, which this desk rejects, and unavailable anyway on one live trade and 53 archived rows. **The ACADEMIC literature was searched separately on 2026-09-13** after item 55 showed that the 09-12 pass had read only vendor and trading-educator material, and it says the threshold is open work. Han, Zhou & Zhu, "Taming Momentum Crashes: A Simple Stop-Loss Strategy" (https://www.cicfconf.org/sites/default/files/paper_811.pdf) is the most-cited empirical stop-loss study; its threshold is a FLAT 10% with 5% and 15% as robustness checks, not scaled to the instrument at all, and it closes with *"we leave the search for optimal stop-loss strategies as future research."* Kaminski & Lo (J. Financial Markets, 2014) is the one general framework, and an academic review of it states the limit: *"Kaminski and Lo (2007), although interesting, give little guidance for the trader who plan to use stop-loss rules to stop losses that occur over a week, a day or a shorter time period"* — the same review explaining the thinness, *"This view may explain the rather limited academic literature on trading rules such as stop-loss"* (Erdestam & Stangenberg, Stockholm School of Economics, http://arc.hhs.se/download.aspx?MediumId=593); that thesis sweeps its own level over a grid, *"γ = 0.5-5%"*. Adopting any of their numbers would import a foreign default — the unsourced act in the other direction, declined by item 55 on the same day. Also do not re-propose "no floor, no trade", falsified 2026-09-12 against George & Hwang (2004) and Bulkowski.

**What would settle it:** a published measurement of stop survival stated as a PROBABILITY rather than a multiple. The reading is built, so only the threshold is missing. Two reformulations needing no threshold, to try first: (i) refuse when the stop's touch probability is below the TARGET's reach probability computed the same way on the same instrument — a probability-form reward:risk test replacing the arithmetic ratio, not sitting beside it; (ii) accept that no threshold exists and DELETE the width gate, since §2.1 sizing already answers a wide stop with a smaller position and the gate refuses at 1.67%. Deletion is a permitted answer and is not the lazy hack — the lazy hack would be switching it off without (a) and (c) above.

**Cost while unanswered:** small, and now quantified. The gate cannot fire on the desk's own fallback and fires only past 6.7x ATR on a three-week trade, so it is close to decoration and the direction of its error is known: it under-refuses. Nothing is wrongly blocked. What remains uncosted is whether anything SHOULD be.
