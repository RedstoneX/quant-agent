...
**53. The stop over a fractional remainder is now re-placed automatically while the desk is paused — BUILT AND TESTED, NOT YET RUNNING ON THE BOX. Narrowed 2026-09-14, found 2026-09-12.** Owner ruled fix-it, not decide-it, so the three options in BOARD_NOTES 53 are closed: the daily path re-places the missing DAY stop instead of only reporting it. What was built: the coverage watchdog now calls the SAME `repair_stop_coverage` the in-session sweep calls (one shared home, no second order path), at the level recorded on the position's own last BUY, gated on the exchange calendar the broker publishes (`is_trading_day` / `get_session_open` / `get_session_close`) and failing closed when that calendar cannot be read. It only ever ADDS a protective order — nothing in it can sell, resize, cancel or zero a position. A placement that fails raises its own owner alert on its own once-per-day marker so an earlier exposure report cannot swallow it. **Why this is still OPEN:** the 06:15 ET alert-heartbeat fires more than three hours before the bell and a fractional DAY stop cannot be placed into a shut market, so the run that actually repairs is a new unit — `scripts/systemd/quant-agent-coverage-sweep.{service,timer}`, the same `*:0/30` tick the desk's own session timers use, self-gating on the calendar. **It is not installed on the production box.** Until someone runs the two-line install in the service file's header, the remainder stays bare exactly as it is today and this fix is code that never runs — which is precisely how item 17c sat dead for ten days. `quant-agent-unit-drift.timer` reports a tracked-but-uninstalled unit as `undeployed`, so the reminder is mechanical, not a note in a file. Close this item when the timer is listed by `systemctl --user -M qamc@ list-timers` and one tick has run. **Not fixed, and not fixable in code:** the remainder is still unprotected OVERNIGHT. This broker accepts a fractional order only as a DAY order (measured 2026-09-01, code 42210000; Alpaca's fractional-trading page says the same), so every re-placement buys one session and lapses at 16:00 ET. The only ways to remove that exposure are to hold whole shares or not to hold the remainder; the owner declined whole-share-only on 2026-09-02. The alert text now says this in those words rather than implying the gap is closed.
