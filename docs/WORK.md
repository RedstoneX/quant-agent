...
DONE WHEN:
  ...
  - [x] (f) the "2+ oversized → cut every BUY 25%" claim stops being unenforced prose — DELETED 2026-09-26, not enforced and not replaced. The 2026-09-26 re-audit found a second, worse instance: `config/prompts/risk_manager.md` stated the cut to the Risk Manager as machinery already running and used it as grounds to discount a conservative PM plan as "anchoring". The feed is real (`_build_rm_recent_verdicts` renders the last 5 verdicts with `cat=` tags into PM's sheet); the cut never was. Both sheets now say so, three sibling sites were corrected, and `config/retired_mechanisms.yaml` pins the phrases so the build fails if either sentence returns.
  - [ ] (f, residue) the other unsourced numbers in the SAME PM section — the 0.25 sizing grid, the `2+ occurrences` trigger threshold, the conviction-to-base midpoints — are class (a) and still open; so is the `_build_rm_recent_verdicts` docstring in `src/pipeline.py`, which still asserts the loop works
