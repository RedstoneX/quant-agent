...
**13. One number was carrying three different meanings. FIXED 2026-09-04,
PR pending review — not yet merged.**

A target weight of 0% used to mean three incompatible things the plumbing
could not tell apart (refuse / close / short) — a rule that refused to BUY
something could silently SELL a position nobody asked to sell. Fixed by
porting `pysystemtrade`'s override algebra: `SizeOverride`
(`src/risk/size_override.py`) makes the intents distinct values
(`no_trading > close > reduce_only > multiplier`) with a combination function
that always keeps the more restrictive one, so the mistake is no longer
expressible in the type rather than relying on a caller to remember a special
case. The one live caller that needed migrating — the 2026-09-02
signed-dissent workaround in `PortfolioConstructor._plan_risk_targets` — now
produces `SizeOverride.no_trading()` instead of dropping a target on a bare
`<= 0.0` float check; behaviour is unchanged (same tests pass). Full detail:
`docs/INCIDENT_HISTORY.md`, 2026-09-04 entry.
