...
**39. Opportunity-cost rotation — nothing may enable the ranked-margin tier while 39(a) is open. Owner-requested; the CATEGORICAL tier is live since 2026-09-12 on the identical path as any close. Detail: `docs/BOARD_NOTES.md` ("item 39").** **Sequenced rather than predicted (attempt 3, PR #563):** the close goes LAST among the session's SELLs and is gated inside the sell loop just before submission. **Stays open: 39(a) is the blocker and says do not promote that number to an execution gate until it is answered.**
...
**97. DELEGATED TO THE ORCHESTRATOR — does the model-guessed holding horizon stay in the pace test? Filed 2026-09-17, ownership moved 2026-09-18.** A number the model states rather than reads is deciding whether a trade is judged to be moving too slowly.
detail: docs/BOARD_NOTES.md (item 97)
...
**108. The position reviewer is told a stop-raise minimum is a hard requirement, and nothing enforces it. Filed 2026-09-18, verified, NOT fixed.** `config/prompts/position_reviewer.md` line 27 states it as a schema rule — `new_stop_price` "must be ≥ `old_stop × 1.02`" — and repeats it twice more as a 2% minimum margin.
detail: docs/BOARD_NOTES.md (item 108)
...
**133. The portfolio manager was never asked why it dropped a candidate, so every refusal read identically and the jam detector could not work — filed and fixed 2026-09-18, item STILL OPEN on the unresolved objection below.** Full account of the defect and the fix: `docs/INCIDENT_HISTORY.md` (2026-09-18).
...
detail: docs/BOARD_NOTES.md (item 133)
...
detail: docs/BOARD_NOTES.md (item 124)
...
detail: docs/BOARD_NOTES.md (item 130)

**131. The standalone coverage sweep may never have run, and nothing would show it — filed 2026-09-18.** Zero `coverage_watchdog` lines appear in the 2026-09-18 production log.
...
detail: docs/BOARD_NOTES.md (item 132)
...
detail: docs/BOARD_NOTES.md (item 159)
