...
DONE WHEN:
  ...
  - [x] the RSG case is re-examined to see which value the seat meant — RESOLVED 2026-09-26, and the answer overturns how this item was filed: **the seat never emitted 0.5 and never contradicted itself.** Stored run `run-601011e0` (2026-09-16) read back read-only from the production database. The portfolio manager's own raw response (`agent_logs.full_response`, the text the model returned) contains the string `0.5` NOWHERE. It asked for a six-name risk ladder stepping down from 2.5 to 1.75, RSG among them at 2.5 — exactly the ladder its `sizing_logic` narrates. The 0.5 that reached the `portfolio_manager` / `target` evidence rows — for RSG and for one other name in that ladder — was written AFTER parsing, by the desk's own deterministic sub-floor size cap: `PortfolioManagerAgent._apply_subfloor_catalyst_rule` as it stood at commit `0d091ae7` (the code live that session), which caps an increase back to `starter_risk_pct` (0.5) when a range setup's reward:risk is under the reference. RSG was a RANGE setup at reward:risk 0.81, under the reference, so the cap fired; the risk seat's own `rr_audit` row for that run carries the same 0.81. (The entry/stop/target triplet that ratio was computed from is real desk output and this repository is public, so it is deliberately not reproduced here — the ratio is the entire input the cap reads, and `rr_audit` is where a reader with database access checks it.) That cap's log line already calls it "Deterministic, not PM inconsistency". RSG was then traded at the capped size rather than the narrated one (`proposed_order`, BUY). So the 2.5% is what the seat meant AND the 0.5% is the correct emitted size — both are right, and the defect is the NARRATION LAG: `sizing_logic` is written before the mechanical adjustment runs and is never restated, so the owner-facing story describes a trade that was not taken. This is the same ordering problem `PortfolioDecision.constructor_dropped` exists to explain to the Risk Manager, on the sizing field instead of the symbol list. Diagnosis only; nothing about the cap or the stored run was changed.
