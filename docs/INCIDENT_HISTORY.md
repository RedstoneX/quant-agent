...
(#262) was never judged by the owner — he was asleep when it got
mechanically auto-closed as a side effect of an unrelated branch
deletion (see `docs/WORK.md` item 40; same cascade as item 32's #259).
Restored on a fresh PR, based directly on `main` this time — the
original was stacked on another PR's branch, which meant its CI never
actually ran (the repo's test workflow only triggers against `main` as
base), a real gap an earlier review caught but couldn't fix from inside
that PR.
