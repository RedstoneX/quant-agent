...
### 2026-09-19 — the risk reviewer could stop or reshape trades on rules it had misread; it is now advisory (owner ruling, board items 162 and 134)

**In plain words:** the AI risk reviewer used to be able to cancel the whole morning plan, refuse single trades, shrink them, move their stops, and hold a sale the desk wanted to make. Two of its three whole-plan vetoes on record cited guideline limits as if they were hard rules, because its own written brief told it to. The owner ruled it "shouldn't have to" block anything: the hard limits are enforced by code. The reviewer now writes down every objection with its reason and the owner sees it ("the risk reviewer objected: …; not applied — owner ruling 2026-09-19"), but nothing it says changes or stops a trade, buying or selling.

**What changed.** Morning plan: the whole-plan veto, per-name refusals, field edits and the "scale every buy" knob are all recorded and none is applied, so the plan after the reviewer is byte-for-byte the plan it was shown (pinned by test). The review's own evidence rows now carry `applied: false`. Four side doors the adversary review found are closed with it: a scale of zero used to drop every buy; an edit over 100% failed the order schema and dropped the name; a stop edit that failed the noise-band re-check dropped the trade; a cut below the minimum risk acted as a refusal. None of those paths is reached any more. Midday and close: the reviewer can no longer hold a SELL, REDUCE or COVER; its objection is recorded as `ai_risk_objected`, not dropped. Its brief was rewritten: it lists the limits code enforces, calls the rest advisory, has no veto instruction and no worked size example (the old 0.7 example is the number it copied every time it scaled, item 134), and carries no number the code does not use.

**Kept, deliberately.** Every hard limit in code runs exactly as before. The code-owned drop of an exit whose stated reason is provably false (the holding-discipline check) stays on both paths. On the morning path it used to share its drop shape with the reviewer's refusal; only the reviewer's half was removed, and a test pins the check still dropping.

**Decided here, and worth checking.** An unreadable reviewer answer used to end the morning with zero orders. That was the 2026-08-27 "fail closed on entries" posture, and it existed only because the reviewer's approval gated execution. An advisory reviewer's absence removes no protection, and failing closed on it would let a garbled answer block the plan, so the morning path now carries on and records the failure. Exits already did this.
