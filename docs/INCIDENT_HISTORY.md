...
**Cause.** The economics numbers were asked for one after another. The first eight, plus their metadata, ate the ninety-second ceiling. The last seven — unemployment, junk and investment-grade credit, real yield, breakeven, the dollar, weekly claims — were never even requested; the log said the deadline was exceeded. That is not a St. Louis outage. Separately, the live snapshot asked for no feed and trusted last-trade only: at 09:30 last-trade can still be yesterday while today's forming bar or quote already carries the open, so eight names (including a holding) were labelled STALE with no open print. The "ages" on the technical reads are how long a rating has persisted, not how old the analysis is, so an eight-day gate on that number would have been invented. The fill socket's login treats the first websocket message as the handshake; a hello or listen-ack before "authorized" is logged as failed login and retried through the review. REST still worked. Starting the socket during review (already shipped) does not help if that first frame is not the login reply. And the 09:30 risk tick is allowed to run while morning still owns the open, so it waited for morning to finish and then sent a second paid look labelled as an intraday opportunity at 09:37 — still the open, not midday.

**Fix.** The miss was serial starvation, not a short ceiling: the existing ninety seconds was enough if every series actually got a try. Ask for all fifteen at once, each with its own request timeout, concurrent enough that every series can still start inside that ceiling. A series that still fails is asked for once more inside whatever budget remains — not a longer clock, and not a made-up number. If it is still missing, name it, skip the economist, and mark the seat lost so the evidence gate refuses. A later repair may re-fetch those series once; it may pay the economist only after they have all arrived. Do not invent the missing prints. Live snapshots pin the feed this account is entitled to and, when last-trade is still yesterday, use today's forming bar or a today quote. The open print is re-read once at 09:30. A name with no today print after that re-read is a lost technical seat, not a low-confidence stain, and is not dropped forever. The fill socket keeps reading until it is authorized, rejected by name, or the existing login budget runs out. REST polling after a failed handshake is labelled a temporary safety net tied to that login fix — not the product, not a longer backoff, and repeg stays off. Reward-to-risk was not invented. A risk tick that still sees morning owning the open does not wait and then fire a second "intraday" pass: morning is the open path; that leftover is skipped with the movers named and no INTRADAY telegram. Midday still waits, then looks. The first true intraday look is a later tick that starts after morning has already released. No minute cutoff was invented.

**What this does not close.** How many usable reads is "enough" is still the owner's (item 20 counting half). A real St. Louis outage still fails closed. A name with no today print at all is still STALE. A second websocket on the same account is still refused. A 09:30 risk tick that starts before morning has written the owner file can still see a free lock; that race is the same hole the wait-then-run path had.

**What would catch it next time.** A test that fifteen series each slow enough to miss a serial ceiling still all get attempted; a test that a series which timed out is asked for once more and lands, and one that still fails stays named with no invented value; a test that one missing required series does not call the economist and marks the seat lost, and that a later repair pays the economist only after a complete re-fetch; a test that a yesterday last-trade with today's open on the bar (or quote) is priced from that today print, and that a first-stale snapshot is re-read into a today print; a test that a parsed technical batch with a STALE live price is lost, not low-confidence; a test that a non-login first frame still authorizes, that an explicit reject fails by name, and that REST after a failed handshake is labelled temporary; a test that a tick overlapping morning does not run a separate paid intraday scan or send INTRADAY OPPORTUNITY, and that a midday wait still runs after that lock releases.
