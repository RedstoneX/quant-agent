...
**Cause.** The economics numbers were asked for one after another. The first eight, plus their metadata, ate the ninety-second ceiling. The last seven — unemployment, junk and investment-grade credit, real yield, breakeven, the dollar, weekly claims — were never even requested; the log said the deadline was exceeded. That is not a St. Louis outage. Separately, the live snapshot asked for no feed and trusted last-trade only: at 09:30 last-trade can still be yesterday while today's forming bar or quote already carries the open, so eight names (including a holding) were labelled STALE with no open print. The "ages" on the technical reads are how long a rating has persisted, not how old the analysis is, so an eight-day gate on that number would have been invented. The fill socket's login treats the first websocket message as the handshake; a hello or listen-ack before "authorized" is logged as failed login and retried through the review. REST still worked. Starting the socket during review (already shipped) does not help if that first frame is not the login reply.

**Fix.** Ask for all fifteen economics series at once, each with its own request timeout, concurrent enough that every series can still start inside the existing ceiling even if each attempt uses a full request timeout — not one-after-another, and not a burst of fifteen. If any required series is still missing, do not pay for a regime built on holes, do not let the later repair step pay for one either, and mark the economics seat lost so the evidence gate refuses — the coverage object still says how many came back; that is not a licence to trade on the holes. Do not invent the missing prints. Live snapshots now ask for the feed this account is entitled to, and if last-trade is not from today they use a today-timestamped forming bar, or a today quote if the bar is not from today either — a real print, never a made-up price. Session range is omitted unless the bar itself is from today. A fully parsed technical batch with no today print on an analysed name is not high confidence. The fill socket keeps reading until it is authorized, rejected by name, or the existing login budget runs out. REST remains the safety net, not the product. Chase/repeg stays off. Reward-to-risk was not invented.

**What this does not close.** How many usable reads is "enough" is still the owner's (item 20 counting half). A real St. Louis outage still fails closed. A name with no today print at all is still STALE. A second websocket on the same account is still refused.

**What would catch it next time.** A test that fifteen series each slow enough to miss a serial ceiling still all get attempted; a test that one missing required series does not call the economist and marks the seat lost; a test that a yesterday last-trade with today's open on the bar (or quote) is priced from that today print; a test that a parsed technical batch with a STALE live price is not "ok"; a test that a non-login first frame still authorizes, and that an explicit reject fails by name.
