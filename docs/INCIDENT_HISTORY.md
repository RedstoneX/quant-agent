...
**Production evidence for item 131, read-only, 2026-09-19 ~01:35 UTC.** Timer active since 2026-09-14 04:47 UTC. The journal holds 235 starts and 235 finishes. The last run exited 0, and no run in the journal failed. The sweep put DAY stops back at the open on 09-14 (ORCL), 09-15 (ORCL), 09-16 (BRK-B, COP, EQNR, NOK, OXY) and 09-17 (AAPL, BRK-B, EQNR, NOK, XOM). It sent one exposure alert, on 09-14 at 21:00 UTC (ORCL, $44.67, delivered). **Its alarm for a failed repair has never fired**, so that path is still unproven in production. On 09-18 at the open it found 9 short-covered positions and placed nothing. Its journal line does not say why. The most likely reason is that the morning session held the session lock, and the new record states the reason. None of this reached `quant_agent.log`, because the process never configured logging: INFO lines were dropped and warnings went to stderr only. The only durable file, `data/alerting/coverage_heartbeat.json`, is overwritten on every run.

**The 2026-09-16 collision, re-read against the broker's own order history.** The sweep created BRK-B's 0.4393-share DAY stop at 13:30:42.121. About 675 ms later, `intra_check` and `morning` (started 5 ms apart) each acted on the same stale gap. They were both refused with `held_for_orders`, both exhausted their retries, and both declared a failure over a stop that was resting at the broker. Neither of them placed the order that landed. The board's account of that incident ("the broker rejected the second") therefore needs correcting: it was a three-process race, and the order that won came from the sweep.

**What changed.** Item 131, observability only: each run logs `COVERAGE SWEEP <run id> (<entry>): started` and a finish line to `quant_agent.log`. The finish line gives the outcome, positions checked, gaps, repairs attempted/succeeded/failed, the alert sent or none, and any deferral reason. Each run also writes one `specialist_evidence` row (`agent_name='coverage_sweep'`, `kind='pipeline_event'`, `scope='run'`). The sweep still repairs the same things, at the same times, with the same thresholds. Item 127: `intra_check`'s broker-writing preamble (the drains, coverage repair, cash-park release and the orphan, stop-out and fill reconciles) now runs only while that process holds the flock its paid scan already used (`.intraday_scan.lock`). It also defers whenever a live morning/midday/close session owns the desk, using the same owner check the paid scan uses. The sweep's repair pass now takes that same flock. When either one defers, only that tick's writes are skipped: the loss check still runs, and the next tick re-reads the broker.

**Still open.** Sessions do not take the flock themselves. A session that starts in the second after `intra_check` or the sweep checked for it can still overlap them. Today that pairing is closed by timing alone: `intra_check` fires at :15/:45, and the sessions and the sweep at :00/:30. The sweep and the sessions share a tick, and on that tick they are separated only by the session-lock check.

**Tests.** `tests/test_desk_broker_write_lock.py` (six) and a new section in `tests/test_coverage_watchdog.py` (five). Reverting the pipeline change fails the two `intra_check` tests, including the drain re-adding a stop that a live morning session cancelled to sell. Reverting the watchdog change fails the two lock tests and the four record tests. Reverting the heartbeat change fails the five record and log tests.
...
### 2026-09-20 — the ranked-margin rotation tier's sequencing gap is closed; the tier itself stays off behind a second, separate switch (item 39)

**In plain words:** the automatic opportunity-cost rotation feature has two triggers for closing a stale holding to fund a stronger one. The first, already live, fires rarely (the holding must fail the desk's own entry rules outright), so on the rare occasion its replacement buy gets refused for unrelated reasons, "sold and did not buy, alert only" was an acceptable outcome. The second trigger, ranking-based, fires far more often and was never turned on for exactly this reason: turning it on with the same behaviour would make "sold and did not buy" routine instead of rare. That gap is now closed, but the ranking-based trigger remains off — closing the gap and deciding to arm the trigger are two different decisions, and this change only makes the first one.

**The fix, in plain terms.** The rotation's sell (of the stale holding) and its replacement buy (the new idea) are already built together and reviewed together, by every risk gate the desk has (hard risk rules, the AI Risk Manager, sizing, correlation, the holding-discipline check), in one batch, before any order is ever sent to the broker — this was already true for the live trigger. What was missing: after that joint review finishes, if the buy did not survive it but the sell did, nothing stopped the sell from still being sent. `_drop_rotation_sell_if_buy_leg_refused` (`src/pipeline_stages.py`, called at the end of `RiskStage._run_review`) now checks exactly that, for the ranking-based trigger only, and withdraws the sell — before it ever reaches the broker — recording a plain skipped event instead of a sale and an alert. No new "dry run" mechanism was built; this reads a result the pipeline already produces. The already-live trigger is untouched.

**Why the trigger is still off.** A new config switch, `execution.rotation_ranked_margin_enabled` (default False), is required in addition to the existing `rotation_enabled` (already True live) before this trigger can fire at all — flipping the existing switch alone does not arm it. This is deliberate: enabling it is a live-order-placement decision for the owner, separate from fixing the sequencing defect. It is also a SEPARATE decision from item 39(a), still open: the 25% score-margin threshold that decides when this trigger fires at all is an unmeasured, provisional number, and arming this switch means selling on that number — the sequencing fix does not answer that question.

**Tests.** `tests/test_rotation_execute.py`: a would-be-refused replacement buy blocks the sell from ever firing; a would-succeed buy leaves the existing sell-then-buy sequencing untouched; the already-live trigger's behaviour is unchanged when its own buy leg is missing; the pre-check is a no-op once the sell has already been submitted this run, and with no active rotation at all. 40 tests in the rotation suite passed at the time (the "60" figure this section originally recorded could not be reproduced against the file as committed and is superseded by the corrected count below).

---

### 2026-09-20 (same day, second pass) — the above fix was INCOMPLETE; an adversary review found three real gaps and one false claim, now closed

**What was wrong with the first pass, and why it still mattered even with `rotation_ranked_margin_enabled` defaulting off.** The fix above only ever intercepts refusals `RiskStage._run_review` itself can produce (hard risk rules, the AI Risk Manager, sizing/correlation, the holding-discipline check). It runs at the END of `RiskStage`, before `ExecutionStage` submits anything — but `ExecutionStage` itself still refuses buys for four more reasons, ALL of them AFTER its own SELL-submission loop has already run: a daily-loss re-check (the tape may have gapped through the limit while PM/RM was thinking), `no_price` (no verifiable live price), `stale_entry` (entry more than 5% from market) and `qty_zero` (the sizing math rounds to nothing). A `ranked_margin` buy refused by any of those four still let its rotation's SELL go out first, reproducing the exact "sold, nothing bought, alert-only" outcome item 39 exists to prevent — the RiskStage-only pre-check closed one door of five.

**Second finding, checked and found FALSE.** The review also claimed the daily-loss re-check's own equity refresh is gated on `if not sell_decisions:` and is therefore skipped on a rotation run (sell_decisions is never empty when a rotation sells), leaving the re-check reading stale, pre-sale equity. Read against `ExecutionStage._run_session`: the refresh that actually feeds the re-check runs unconditionally on `if sell_decisions or cover_decisions:` — true precisely when a rotation's SELL exists — and that refresh already updates `ctx.total_value` before the re-check runs. The `if not sell_decisions:` branch only guards a SECOND, otherwise-redundant refresh that exists for the buy-only case (no SELL this run, so the first refresh never fired). No code change was applicable here. `test_post_sale_equity_refresh_is_not_skipped_on_a_rotation_run` (`tests/test_rotation_execute.py`) pins the refresh firing exactly once and the daily-loss check reading the refreshed, post-sale total on a real sell+buy run — it fails if that gating regresses in the future.

**Third finding, real: the structural barrier was weakened, not just supplemented.** Before item 39, `rotation_sell_reason` raised `ValueError` for the `ranked_margin` tier — a `ranked_margin` sell reason could not be BUILT at all, so nothing downstream could ever act on one; the only way to change that was a code change. The first pass replaced that with `execution.rotation_ranked_margin_enabled`, a plain config boolean defaulting False. That boolean alone was already sufficient, structurally, to let a `ranked_margin` sell reach the broker if flipped — a real regression from "cannot happen without a code change" to "can happen with one config edit," independent of whatever else the sequencing pre-check verified.

**The fix.** `_rotation_ranked_margin_execution_gate_failure` (`src/pipeline_stages.py`) evaluates a `ranked_margin` rotation's replacement buy against the SAME four `ExecutionStage` gates — daily-loss re-check, `no_price`, `stale_entry`, `qty_zero` — using the same helpers and thresholds those gates use further down in the same method, but using PRE-sale `ctx.total_value`/`ctx.positions` (the sale has not happened yet). Pre-sale equity is the conservative side for the loss check (worse of the two whenever the sale itself is not a loss) and a reasonable stand-in for sizing (a same-day round-trip swap changes total value only by slippage). `_drop_rotation_legs_if_buy_would_fail_execution_gates` calls it at the very top of `ExecutionStage._run_session`, before the HOLD loop, before the SELL loop, before anything is submitted: on failure it withdraws BOTH the sell and buy decisions from the plan together and records why; on success it stamps `ctx.rotation["ranked_margin_precheck_passed"] = True`. The SELL loop's own submission point now additionally requires that stamp before it will submit a `ranked_margin` close — not `rotation_ranked_margin_enabled` alone. This is the structural-barrier fix for the third finding: a future code path that forgets to call the pre-check (a refactor, a new call site) hits this guard and is refused, fail-closed, the same posture every other rotation guard in this file already takes. `ineligible_hold` is untouched by any of this — it keeps the sell-then-alert tolerance it has always had.

**What remains open, honestly.** A broker-accepted `ranked_margin` SELL that simply does not fill, or partially fills, is real market-fill uncertainty that no pre-check can predict — `_drop_rotation_buy_if_room_not_freed` already refuses the buy correctly in that case and pages the owner via the existing "sold, nothing bought" alert; that residual path is deliberately NOT made unreachable, because it cannot be, and closing it was never this item's claim. Item 39(a) (the 25% score margin) is unchanged by any of this.

**Tests.** `tests/test_rotation_execute.py`, new section: `test_execution_gate_fails_on_daily_loss_recheck`, `..._no_price`, `..._stale_entry`, `..._qty_zero` and `..._passes_a_healthy_buy` pin the four gates individually; `test_wrapper_withdraws_both_legs_when_the_buy_would_fail_a_gate` and three more pin the wrapper's withdraw/no-op/pass-recording behaviour; `test_ranked_margin_sell_never_reaches_broker_when_buy_would_fail_daily_loss_recheck` and `..._has_no_price` prove the SELL never reaches the broker end to end through the real `ExecutionStage`, and that the "sold, nothing bought" alert never fires for a sell that never happened; `test_ranked_margin_flag_alone_is_not_sufficient_when_precheck_never_ran` disables the pre-check and proves the SELL loop's own hard guard still refuses the order even with the config flag on; `test_ranked_margin_sell_proceeds_once_the_precheck_recorded_a_pass` is the positive control; `test_post_sale_equity_refresh_is_not_skipped_on_a_rotation_run` is the second finding's proof. 55 tests in `tests/test_rotation_execute.py` pass [measured]; `tests/test_pipeline_stages.py` + `tests/test_pipeline.py` + `tests/test_portfolio_constructor.py` (287 tests) pass unchanged [measured]; `tests/test_definition_of_done.py -k this_change` (4 tests) passes [measured].
