...
**Fix.** Book the winner's cost; mark the day inexact so the booked total is a known minimum; do not increment the unknown-row counter and do not latch. Persist the reasoning chain and the live sector rows so a same-day snapshot can re-validate; if it still cannot, record a durable missing-field reason — do not loosen validation. Keep a stated soft-exit; record `unknown` when an actionable seat sends JSON null; do not invent a falsifier. Union Form-4 admission-eligible names into the filing check with no new name-count cap. Page a given mismatch fingerprint once per Eastern day; still log every time; never copy the broker into the archive. Place the DAY remainder first; if the broker says shares are held, treat an already-live covering stop as success. Wait for morning/midday to finish instead of sleeping the scan until the next half-hour; a finished session's fills are not an in-flight lock.
...
**What would catch it next time.** Tests that a known-cost winner after an ambiguous retry does not hard-latch, that a stored economics snapshot with a chain re-parses and one without names the missing field, that null is recordable don't-know and a stated string survives, that hot insider names join the filing check, that an identical mismatch does not re-page the same day, that a held_for_orders DAY remainder is covered when the broker already holds it, and that a finished session's trade rows do not skip the scan.
