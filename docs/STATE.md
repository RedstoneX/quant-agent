...
- **FIXED 2026-09-17 — morning-open research path, measured on live paper.** Incomplete FRED (any required series missing after a real fetch) is a lost economics seat: no regime is paid for on the holes, and the evidence gate refuses. The coverage object may still say "partial"; that is a count, not a licence to size off a thin book. Live snapshots pin IEX (the feed this account is entitled to) and, when last-trade is still yesterday, use a today-timestamped forming bar or quote — a real open print, never an invented price. A fully parsed technical batch with no today print on an analysed name is not high confidence; rating persistence is not analysis age, so no eight-day clock was invented. `trade_updates` login drains non-auth frames until authorized or named-rejected; REST polling remains the fallback, not the product. Item 20's counting half is unchanged.
