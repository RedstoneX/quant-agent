...
- the cockpit default is chart-first: NLV/sessions are one-line collapsed chrome (liquidity and the decision banner remain behind “Show full header”), Holdings is a Dockview panel above the chart that the operator can move, dock and resize, and the chart pane is the largest region;
- Holdings cards keep their size, wrap at four per row on a normal desktop (fewer on a narrower window), and never use a horizontal slider; on iPad/phone the same cards remain a header strip;
- workspace vertical sashes work both ways: the sash above the chart borrows from the row above (Holdings by default), the sash below borrows from Positions/Orders then grows the page past that floor; extra height persists across reload;
