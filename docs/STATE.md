...
- the cockpit default is chart-first: Holdings/NLV/sessions are one-line collapsed chrome (liquidity and the decision banner remain behind “Show full header”), and the Trading Workspace opens with the chart pane as the largest region;
- the Holdings strip is collapsed to a count/P&L line by default; when expanded, cards keep their size, wrap at four per row on a normal desktop (fewer on a narrower window), and never use a horizontal slider;
