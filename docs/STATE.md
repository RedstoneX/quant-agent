...
- **FIXED 2026-09-17 — morning-open research path, measured on live paper, retested mid-morning.** Incomplete FRED (any required series missing after a real fetch and one bounded re-ask inside the same ceiling) is a lost economics seat: no regime is paid for on the holes; a later repair re-fetches the series and pays the economist only when they have all arrived. The coverage object may still say "partial"; that is a count, not a licence to size off a thin book. Retest vs the open: isolated series land in 1.5–6.7s; a healthy full batch can make 15/15 in ~85s when not cascading; the open miss was 7/15 never attempted. The remaining root is observation+metadata sharing worker slots under the existing 90s ceiling, not a short clock — observations are attempted first (one try, then one bounded re-ask of misses) so every required series gets a real try; metadata is remaining-budget and unknown freshness is named; values are never invented and the timeout is not lengthened. Live snapshots pin IEX. When last-trade is still yesterday, today's forming bar or today's IEX minute bar is the open print — a quote is not a print. Mid-morning 6/8 recovered; CHPX+DXPE stayed IEX-thin with no print and a fast API: that is lost tech, not fresh confidence. `trade_updates` login drains non-auth frames until authorized or named-rejected; the hub starts at the portfolio-manager step so handshake is not the open Risk window (the storm cleared mid-morning; open Risk is still the defect). REST polling after a failed handshake is a labelled temporary safety net, not the product. The 09:30 paid open is morning only. An `intra_check` fire at that shared open (including a leftover still on the same half-hour cadence) does not buy a second hunt and does not wait-then-run as INTRADAY: movers are named, Telegram stays silent, and deterministic risk/coverage still run. The first true paid INTRADAY look is the next existing half-hour fire after morning released — finish stamp when the once-day marker was written, lock release when it was not. No pad after 09:30 was invented. Deploy copies systemd units from the checkout rather than restarting the API alone. Item 20's counting half is unchanged. Repeg stays off.
...
- **Phase 2b (risk-based sizing) merged and deployed** — `75c0233` on branch
  ...
  **Two follow-on fixes landed the same day, same branch, also merged and
  ...
  (`risk.min_stop_atr_multiple`, base 2.5 since 2026-09-10 — was 1.5
  (2026-09-04, from this desk's own Maximum Adverse Excursion data, later
  found to overlap the window some seats misreported confidence/data
