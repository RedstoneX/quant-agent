...
- **FIXED 2026-09-17 — morning-open research path, measured on live paper.** Incomplete FRED (any required series missing after a real fetch and one bounded re-ask inside the same ceiling) is a lost economics seat: no regime is paid for on the holes; a later repair re-fetches the series and pays the economist only when they have all arrived. The coverage object may still say "partial"; that is a count, not a licence to size off a thin book. Live snapshots pin IEX and, when last-trade is still yesterday, use a today-timestamped forming bar or quote — a real open print, never an invented price — and re-read that print once at the open. A fully parsed technical batch with no today print on an analysed name is lost, not low-confidence; rating persistence is not analysis age, so no eight-day clock was invented. `trade_updates` login drains non-auth frames until authorized or named-rejected; REST polling after a failed handshake is a labelled temporary safety net, not the product. An `intra_check` tick that still sees morning owning the open is leftover of that session, not a separate INTRADAY OPPORTUNITY: paid discovery is skipped with the movers named, Telegram stays silent, and the first true intraday look is a later tick that starts after morning has already released. No minute cutoff was invented. Item 20's counting half is unchanged. Repeg stays off.
