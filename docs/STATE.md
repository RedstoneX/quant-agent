...
**Update 2026-08-12, later:** operator entered all four real credentials as Custom Secrets in OneCLI (`dev` never saw the values). All four now verified working end-to-end through the gateway (OpenRouter, Alpaca trading, Alpaca data, FRED — see `docs/architecture/CREDENTIAL_DELIVERY_EVIDENCE.md`), after the operator fixed three real configuration gaps `dev` diagnosed empirically: secrets not yet granted to the Default Agent, Alpaca's host pattern missing `data.alpaca.markets`, and Alpaca's header value format incorrectly carrying an OAuth `Bearer` prefix. Zero `src/`/`config/` changes were needed for any of the four. Remaining step: wire `qamc`'s `.env` (`HTTPS_PROXY`/`SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE`) — operator-only, `dev` cannot write into `/home/qamc` — then verify `broker_reachable` flips true on the live `/health` endpoint. Trading timers remain disabled.
