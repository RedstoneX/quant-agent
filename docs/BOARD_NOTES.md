...
**Progress, 18 September, second pass — the standard was applied to the OTHER messages.** The evening report had been redesigned to your review and signed off, and nobody applied that standard anywhere else. The pre-market filings message you received at 08:03 ET still had every fault the evening one had. Two changes shipped.

**The first is deletion, and it covers every message the desk sends, not just that one.** The run identifier is gone everywhere, including from the stored-report read-backs, which carried their own copy of it. So is the provider-request count. "status: Preprocessed" is gone: an internal code with its underscores taken out is still an internal code, so the outcome now reads as a phrase, and a code nobody has plain wording for is reported as exactly that with the code labelled as the desk's own rather than passed off as a sentence written for you. "elapsed: 3m 5s" is now "took 3m 5s". Scheduler names are gone from the text: "earnings_preprocess" is "Pre-market filings", "intra_check" is "Half-hourly check". Cost is stated in words through ONE helper — the evening message's existing wording, moved so every message calls the same code and a second style cannot grow. "$0.0000" was truthful, because the seats that run are priced at zero on purpose, but it read as broken; it now says "none — this run used only free models", and a cost that cannot be read says "not available" and is never rendered as a zero or a guess.

**The second is substance.** "analyzed: 1, confirmed: 1, failed: 0" now names the company, says what kind of report it was and when it was filed, what the desk concluded about it, and whether that was saved for the morning session to use. Where the desk recorded no verdict, the message says so rather than inventing a direction. Where the desk recorded no NAMES at all, the message says that too, instead of falling back on the counts. The same was done to the quarterly self-review, which used to say "applied=3 rejected=1" and now says what changed and what did not and why.

**Every failure, skip and refusal now says who, what, why, when and what it means for you.** Which company, by symbol and company name where the name is on file; what was being attempted, in plain words; why it failed; when; and — the part that was always missing — whether anything is now unprotected, unbought, unsold or stale, and whether you need to do anything. Most of the time the honest answer is that nothing needs doing, and it now says so rather than leaving you to guess. A raw fault message is kept where it is the only record of what broke, but labelled as machine text with a note that there is nothing in it for you.

**Five of the thirteen clarity defects are now closed across the board** (they were fixed in the evening message only): run identifiers, internal status codes, raw machine text reaching you unlabelled, a missing reason on a rejection, and bare tickers with no company name in the filings message. Two more are partly covered: percentages with no denominator (the daily-spend line dropped its percentage, which only restated the two figures beside it) and detail truncated mid-sentence (every clip in the new text breaks on a word).

**One thing this could not fix, and you should know about it.** When a session CRASHES, the desk records nothing about which companies it had reached. The message says that in as many words rather than guessing at a name. That is a gap in what the desk WRITES DOWN, not in what it says, and closing it is separate work. One nearby half of it was closed: when a single filing failed while others succeeded, the desk knew which one and why and wrote it only to a log file, so the message could never say more than "failed: 1". It now carries that out. What it still does not have is a plain-English reason — only the machine fault text — so the message labels it rather than paraphrasing it into something it cannot support.
