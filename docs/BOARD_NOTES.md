...
**Plain language —** You ruled: no floor, no trade — and no ceiling is fine. A "floor" is a price the stock has already bounced off at least twice, sitting below where we'd buy. It is where the stop goes. Without one, the stop is just a distance somebody picked and hoped about, and that is the trade you refuse to hold. A "ceiling" is the same thing above the price; a stock at new highs has none by definition, and that is exactly the breakout you want — it is managed by the trailing stop, not by a target overhead. Before this, the desk did not require a floor at all: if the analyst typed a stop, the desk accepted it, and if nothing on the chart backed it, the desk simply pushed it out to a "typical day's range" distance. Now a trade with no floor is refused, the refusal is written down by name so it shows up in the "why didn't we trade" counts, and a trade with no ceiling is left alone. This applies to both kinds of setup — a breakout still needs support beneath it. One thing had to be fixed first: the desk only ever looked for floors within a flat 40% of the price, a number nobody derived. On a quiet stock 40% is months of movement; on a wild one it is a week. Now the desk asks how far THIS stock can plausibly travel, using its own daily range — the same arithmetic it already uses to decide whether a target is reachable — so a floor a volatile stock can genuinely reach is never thrown away for sitting past an arbitrary line.
**Example —** A stock trades between $45 and $50 for months, then gaps to $80 on news. The first version of this rule would have taken the old $50 shelf as the floor — the gap had just blown up the stock's measured daily range, so $50 still looked "reachable", and a buy at $80 would have carried a stop near $50, 37% away. You ruled that out: nobody bought or defended anything between $50 and $80, because the market never traded there — it jumped over it. So a level on the far side of a gap that has not been filled does not count as a floor. On the gap day the stock therefore has NO floor and the trade is refused. Two and a half weeks later it has settled between $74 and $80 and bounced off $75 twice. That $74.40 floor sits above the gap, so it counts: a buy at $79 carries a stop just under $74.40, about $4.60 of risk a share, at normal size. If instead the stock had fallen back and traded down through $50 — filling the gap — the old shelves would count again the same day, automatically. Nothing waited for a calendar; the chart answered on each day.
**The decision —** Yours, made. Recorded here so it stops coming back as a question. Two things were deliberately NOT added: no waiting period after a gap, and no filter trying to tell a "real" catalyst gap from a speculative pop — the floor requirement already does that work, and a volume-based test is unproven.
**Recommendation —** Nothing needed from you; both rulings are built and pinned by tests. One inherited detail, stated so it is not mistaken for a derived number: the desk's existing gap detector — the same one that already lists unfilled gaps in the analyst's briefing — ignores gaps smaller than 2%. That threshold predates this rule and was never derived for it; it was reused rather than writing a second definition of "gap". A gap under 2% is a normal day's wobble on most names and would not have re-priced anything, so it is unlikely to matter, but it is a number someone once chose.
||||||| c929e5d
