...
**Plain language —** BUILT, except for one question that is yours. The desk can only risk a quarter of the account in total. Removing the rule that was wrongly blocking good trades roughly doubled how many trades the desk is allowed to take, so that ceiling now runs out on an ordinary day — the desk wants to risk about twice what it is permitted to. Something had to decide which trades get the money, and until now nothing did: it went to whichever trade had asked for the BIGGEST amount. That is a measure of size, not of quality, so a mediocre idea asking for a lot beat an excellent idea asking for a little, every single time. Your decision — best first — is now what happens: the strongest idea gets funded, then the next, until the money runs out. It uses the ranking the desk already computes and already shows the decision-maker, so no new scoring was invented. One smaller thing was found and fixed on the way: a trade that missed out purely because the money ran out was leaving no record of why — it just disappeared off the list. It now says so by name.

**Example —** On the one real day with good records: 25 trades allowed, together asking to risk about 48% of the account, ceiling 25%. Worked through at the average ask of about 1.9% each, 13 of the 25 get funded and 12 get nothing. That count is the same as before. What changed is WHICH 13 — before, the 13 that had asked for the most; now, the 13 the desk rates highest.

**The decision —** One question left, and it is about money so it is yours. **If the next-best idea only PARTLY fits what is left, does it get taken at the smaller size, or skipped?** Say the 13th-best idea wants 1.9% and there is 0.9% left. Option one: it takes the 0.9% — a real position, comfortably above the desk's minimum size, but smaller than its own thesis asked for. Option two: it is skipped, and the 0.9% goes to a lower-ranked idea that fits in full, or is simply not spent.

**What is running right now —** Option one, the smaller size. That is deliberately the *least* change: it is exactly what the desk has always done with a trade that did not quite fit, so what shipped this week changes the ORDER the money is spent in and nothing else. If both had changed at once, next week's numbers could not tell you which change did what. Switching to option two is a one-line change whenever you want it.

**Recommendation —** None offered on purpose. Both answers are defensible and neither is a fact about the market: option one spends the whole budget every day and accepts that the last position's size comes from what happened to be left over; option two refuses to hold anything at a size nobody chose, and accepts leaving money idle. Which is right depends on whether a shrunken version of a good idea is worth owning at all, and that is a judgement about what kind of desk this is.

**Also worth knowing, no decision needed —** the practice runs (backtests) still pick trades in alphabetical order when the budget runs out, because they have no ranking to work down. That is now on the board in its own right (item 62). It means a backtest on a busy day does not describe the desk that actually runs now — so neither the old rule nor your new one can be judged by backtesting until it is fixed. Nothing was invented to paper over it.

## item 52

**Plain language —** The desk watches company insiders buying their own stock, and throws away any purchase below a flat dollar figure — $100,000 for names we already follow, $250,000 for everything else. Nobody knows where those two figures came from; they are not in any research. This was previously written up as a question for you about buying insider-holdings data. That framing was wrong, and it has been corrected: the paper we were leaning on measures a filer's whole quarter added up in one stock, not one purchase at a time, so it cannot tell us where to draw a per-purchase line at all — relative or absolute. No size gate was built on 13 September for exactly that reason, and that restraint was right. What is left is a research question, not a decision for you.
...
**Plain language —** A "level" is a price the stock has bounced off before, and the desk uses them for almost everything — where to put a stop, whether a trade is worth taking, how big it can be. Two things define one, and neither has a source behind it. First, what counts as a bounce: the desk requires a low with three higher days either side in one place and five in another. Second, how wide the level is: the desk treats any two bounces within 1% of each other as the same level. On 13 September the published literature was read properly for the first time on this, and it does not answer either question — the standard five-bar setting became standard because it shipped as a default in a piece of trading software, not because anyone measured it. Both numbers were kept and honestly labelled rather than quietly swapped for each other.
**Example —** On a $200 stock, 1% makes the level $2 wide. Two bounces at $198.10 and $200.00 are "the same level"; bounces at $197.90 and $200.00 are two different levels. That single call decides whether a stop counts as sitting on real structure — and a stop that does gets honoured as-is, while one that does not gets pushed wider, which shrinks the position. So an unsourced 1% is quietly sizing trades.
...
**Recommendation —** The strongest outcome would be getting rid of the percentage entirely — letting each level's width come from how far apart its own bounces actually were, so the stock tells us the width instead of us telling it. Worth trying that before hunting for a study.
