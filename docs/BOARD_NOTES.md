...
**FIXED 2026-09-25:** `_await_paid_scan_slot` (`src/pipeline.py`) now distinguishes which session it waited on. When morning released the lock, the tick stays skipped (new `intraday_scan_open_overlap` status, `_intraday_open_overlap_skip`) instead of running paid discovery on the same tick — the boundary is the lock's own live release, not an invented minute offset, so the next existing half-hour fire (which sees no lock at all) is the first real paid INTRADAY look. Midday/close waits are unchanged (different cadence than this fire, so running on release there is still correct). `intraday_scan_open_overlap` was added to `trader_feed._INTRADAY_SILENT_STATUSES` so no INTRADAY OPPORTUNITY message goes to the owner for it. Rehearsal-rig statuses from PR #435 deliberately NOT carried over, per the note above.
