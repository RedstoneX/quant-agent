...
**Technical detail (item 39), moved from `docs/WORK.md` 2026-09-24 —** The original defect: the tier sold first while the replacement buy was still refusable downstream — a naked sale, alert only (adversary 2026-09-17). Attempt 1 downgraded a hard barrier to a config boolean; attempt 2 (#555) checked PRE-sale state, which cannot work because the gates it anticipates recompute from the held book AFTER the sale. Attempt 3 (PR #563) re-reads the other SELLs rather than guessing, and only the unmade sale is projected, through the same `_entry_deployment_budget` and sizing helpers execution really runs; the barrier is an object carrying those numbers (`RotationClearance`), not a config flag. FIVE covered refusal paths remain — it was six until the owner's removal of the account-level loss halt (#584) deleted the `daily_loss_recheck` refusal outright — four uncoverable by construction; a COVER in the same session refuses the rotation outright (`docs/INCIDENT_HISTORY.md`, 2026-09-23).

**39(a) technical detail, moved from `docs/WORK.md` 2026-09-24 —** How much better must a candidate's composite verdict score be than a holding's before a swap beats round-trip cost, in verdict-score units? Ruled out, do not repeat: Grinold & Kahn, FTSE Russell banding, rebalancing tolerance bands, backtesting, and (2026-09-17) the models' own confidence scores and entry-price-versus-now. 2026-09-23: the comparison had never been REACHED (all 51 rendered sections said "real room exists"); the precondition is now the union of every binding limit and each refusal records its ratio and both sides (`docs/INCIDENT_HISTORY.md`, 2026-09-23).
...
**RULING 2026-09-24 (owner) — no-churn, ranked-margin tier off by mandate.** Rex ruled that a thesis-intact holding is never sold merely because a candidate scores higher; opportunity cost is not a fourth sell trigger. The ranked-margin (tier-2) rotation is therefore off by OWNER MANDATE, permanently, not merely pending the unsourced 25% margin. Item 39(a) (the research question of how much better a candidate must score) is DISSOLVED: holding discipline (retired item 25) keeps a thesis-intact eligible position unsellable on opportunity cost at ANY margin including zero, so no number can open the tier. The 0.25 `ROTATION_MARGIN_PCT` and `rotation_ranked_margin_enabled` flag are now dead config (flag stays False); removing them and their number_ledger entry is a safe follow-up. The categorical tier and the #614 `held_below_entry_bar` telemetry are unaffected.
...
**Plain language —** When the desk cannot read a stock's recent choppiness (its volatility, the number it normally uses to place a protective stop), it must NOT give up on protecting the trade and it must NOT invent a made-up stop. There are almost always price levels to lean on — a recent floor the stock has bounced off, or simply yesterday's low — so the desk reads the stop from one of those and keeps the position. It only walks away from that one name if there is genuinely no level to read at all, or the nearest one is so far below the buy price that the trade would risk more than the desk allows.

**Resolved 2026-09-25 (owner ruling) —** "A missing volatility reading is never a reason to skip protection or invent a fixed stop. There are always levels, even from a few days ago, and there are other ways of setting a stop." An earlier pass had made this case REFUSE the name outright; that was overruled. The stop is now derived from price structure the desk already computed (the nearest trusted support/resistance on the protective side, else the signal/prior bar) and the position is HELD; the name is skipped only when no level is readable at all, or the only readable level sits past the desk's stop-distance sanity bound (a skip on risk, not on the missing reading). The buffer placed past the level is owner-appetite and ledgered with its open question. Published basis: swing-low, prior-bar low and Donchian channel-low stops.
