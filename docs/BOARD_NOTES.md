...
**What that meant in practice.** The macro seat is the only one that back-fills a stance onto every name: where the read stated nothing about a name's sector, the broad market outlook was written into that name's slot and then counted as an independent per-name vote. One market opinion, counted once per name. So: a BROADCAST stance no longer counts in the agreement tally in either direction, a SECTOR-SPECIFIC stance counts in full exactly as before, and a name whose only supporting seat is macro is refused at the conviction bar whatever strength that reading claims. The strength itself is not a new number — it is the seat's own stated `confidence`, which the candidate ranking already weighted; a second scale keyed on that same field is the double count retired item 31 already deleted, and it was not reintroduced.

**Why the broadcast exclusion is sign-symmetric rather than a penalty.** It is one removal that the aligned count and the opposed count consult identically, so a broadcast stance stops corroborating a long by exactly as much as it stops dissenting against it. The opposition side of the conviction bar already made this carve-out (`_is_broadcast_macro_verdict`); leaving the SUPPORT side counted was the asymmetry.
