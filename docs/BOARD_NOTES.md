...
**The decision —** None for you. **RESOLVED 2026-09-25 —** no published study supports single-transaction dollar size as a positive insider-buy predictor (the closest, Cziraki & Gider 2019, finds size inversely related, not positively). Size gates nothing: both flat dollar cutoffs are deleted from the code and config, so a purchase is never discarded for being under $100,000 or $250,000. Relative size (the ratio to the filer's own holdings) is unaffected and still reported on every observation.

**Moved from WORK.md (2026-09-24) —** Settles with a published study of a SINGLE transaction's predictive content by size, or the conclusion that size gates nothing and both dollar filters are deleted. **Settled 2026-09-25 by the second branch: no such study exists, both filters deleted.**
