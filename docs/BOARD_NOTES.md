...
**(f) re-verified 2026-09-26 and it is worse than "unenforced prose".** `config/prompts/portfolio_manager.md:643` tells the PM to cut every BUY base 25% on a second `oversized` tag. `config/prompts/risk_manager.md:41` then tells the RISK MANAGER that this loop already runs, and draws a conclusion from it: that a conservative-looking plan may be anchoring on the risk manager's own history rather than expressing conviction. No code reads a count of `oversized` tags or scales anything from one, and the 25% is not in `config/number_ledger.yaml`. So one seat is instructed to do something nothing enforces, and a second seat is told to discount the first seat's output because of it. Filed, not fixed and no value chosen: enforcing, sourcing or deleting a sizing instruction in the sheet that sizes trades is a money decision.

**Still open under this item:** (a) the un-enumerated remainder of the ~55 prompt-only numbers and ~20 unsourced market claims — the PM and technical sheet subsets are already catalogued precisely under item 107 and should be resolved there, not re-listed; (b) the technical seat's five unnamed data blocks, untouched here; (c) the dead-weight prose in the PM, risk manager and position reviewer sheets, untouched here; (f) above; (g) rendering-or-pinning every code-controlled sentence, which is item 107's registry and covers two pairings so far.
