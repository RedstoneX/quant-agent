...
**Example —** A stock trades between $45 and $50 for months, then gaps to $80 on news. On the gap day its measured daily range has just been blown up by the $30 jump, so the desk's "how far can it travel" window is about $41 — and the old $50 shelf, 37% below, is still inside it. It is the only floor, so a buy at $80 would carry a stop near $50: real, but so wide that at your 5% risk cap the position is tiny (about $13,600 of a $100,000 account). Two and a half weeks later the stock has settled between $74 and $80 and has bounced off $75 twice. The gap has faded out of the daily-range measure, the window is now about $28, the old $50 shelf has fallen out of reach, and the new $74.40 floor is what the desk sees. A buy at $79 now carries a stop just under $74.40 — about $4.60 of risk a share — and a normal-size position. Nothing waited for a calendar to run; the chart itself answered the question on both days.
...
**Recommendation —** Nothing needed from you. Worth knowing only that "no floor on the gap day" is not literally what happens when a stock gaps from an established base: the old base still counts, and the trade is allowed at a very small size rather than refused. If you would rather that trade be refused outright, that is a separate ruling — the window that permits it is the stock's own measured volatility, not a number anyone chose.
