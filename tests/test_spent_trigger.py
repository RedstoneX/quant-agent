...
def test_a_second_cut_citing_nothing_beyond_the_trigger_name_is_spent():
    """"adverse news" as evidence of adverse news names no new record, so it
    cannot be new information."""
...
def _pipeline_with_acted(position, acted_rows):
...
def test_the_prompt_no_longer_permits_the_repeat():
    ...
    # The unconditional permission the item filed against is gone.
    assert "ONLY when one of these HARD\n   triggers fires" not in text
