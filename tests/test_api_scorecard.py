...
def _credit(**kw) -> str:
    payload = {
        ...
        "stance": "buy", "conviction": "high", "weight": 1.0,
    ...
    if populated:
        ...
        _insert(db, agent="news", kind="conviction_credit", symbol="AAPL",
                payload=_credit(seat="news", side="opposed", stance="bearish",
                                conviction="low", weight=0.3, credit=-0.6,
                                nominated=False))
        ...
        _insert(db, agent="news", kind="conviction_credit", symbol="MSFT",
                ...
                payload=_credit(seat="news", symbol="MSFT", side="opposed",
                                stance="bearish", conviction="medium", weight=0.6,
                                r_multiple=-1.0, credit=0.6, position_id="pos-2",
...
def test_scorecard_returns_per_analyst_record_with_raw_counts(tmp_path, monkeypatch):
    ...
    news = by_name["news"]
    # The dissenter on the losing trade is paid for having been right.
    assert news["cumulative_credit"] == 0.0
    assert news["calls_right"] == 1
    # Never off its own best, so nothing is claimed about a drawdown start.
    assert news["below_best"] == 0.0
    assert news["below_best_since"] is None
    assert news["calls_since_peak"] == 0
...
def test_idea_trace_carries_both_sides_and_each_analysts_own_reason(tmp_path, monkeypatch):
    ...
    assert aapl["opposed"][0]["credit"] == -0.6
...
def test_projection_matches_the_ledgers_own_aggregate_when_it_is_available():
    ...
    rows = [
        {"analyst": "technical", "symbol": "AAPL", "side": "supported", "stance": "buy",
         "conviction": "high", "weight": 1.0, "r_multiple": 2.0, "credit": 2.0,
        ...
        {"analyst": "technical", "symbol": "MSFT", "side": "supported", "stance": "buy",
         "conviction": "high", "weight": 1.0, "r_multiple": -1.0, "credit": -1.0,
         "resolved_at": "2026-07-14 15:00:00", "position_id": "pos-2",
         "decision_id": "d-2", "direction": "long", "nominated": False},
        {"analyst": "news", "symbol": "MSFT", "side": "opposed", "stance": "bearish",
         "conviction": "medium", "weight": 0.6, "r_multiple": -1.0, "credit": 0.6,
         "resolved_at": "2026-07-14 15:00:00", "position_id": "pos-2",
         "decision_id": "d-2", "direction": "long", "nominated": False},
    ]
    mine = {
    ...
    theirs = ledger.aggregate_seat_records([
        ledger.SeatCredit(
            ...
            conviction=r["conviction"], weight=r["weight"], r_multiple=r["r_multiple"],
