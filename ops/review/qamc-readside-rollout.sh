...
[[ "$SELF_OWNER:$SELF_GROUP:$SELF_MODE" == "root:root:700" ]] \
  || die "script must be root:root mode 0700"
...
[[ "${DIR_MODE: -2:1}" =~ ^[0145]$ && "${DIR_MODE: -1:1}" =~ ^[0145]$ ]] \
  || die "script directory must not be group/world writable"
...
health_field() {
  health_json | python3 -c 'import json,sys; print(json.load(sys.stdin).get(sys.argv[1]))' "$1"
}
wait_healthy() {
  local deadline=$((SECONDS + ${1:-90}))
  while (( SECONDS < deadline )); do
    if [[ "$(health_field status 2>/dev/null || true)" == "ok" ]]; then return 0; fi
    sleep 2
  done
  return 1
}
...
COUNT="$(printf '%s\n' "$CHANGED" | grep -c . || true)"
[[ "$COUNT" == "$EXPECTED_CHANGED_FILES" ]] || die "unexpected production delta file count: $COUNT"
FILELIST_SHA="$(printf '%s\n' "$CHANGED" | sha256sum | awk '{print $1}')"
[[ "$FILELIST_SHA" == "$EXPECTED_FILELIST_SHA" ]] || die "production delta file-list hash mismatch"
qgit "diff --quiet '$BASELINE_SHA' '$TARGET_SHA' -- config/settings.yaml" \
  || die "target unexpectedly changes committed settings.yaml"
[[ "$(committed_intraday_state "$TARGET_SHA")" == "false" ]] || die "target committed intraday default is not false"
qgit "grep -qF '@router.get(\"/quotes\"' '$TARGET_SHA' -- src/api/routes_live.py" \
...
PRICE_JSON="$(curl -fsS --max-time 30 "${API_BASE}/prices/SPY?lookback_days=5")"
printf '%s' "$PRICE_JSON" | python3 -c '
import json,sys
d=json.load(sys.stdin)
assert not d.get("error"), d.get("error")
assert d.get("symbol")=="SPY"
assert len(d.get("bars") or []) > 0
' || die "/prices did not return historical SPY bars"
...
as_qamc "grep -q '^TELEGRAM_BOT_TOKEN=${TG_PLACEHOLDER}\$' '$QAMC_ENV'" \
  || die "Telegram runtime env no longer contains only the OneCLI placeholder"
...
[[ "$(health_field status)" == "ok" && "$(health_field paper)" == "True" ]] \
  || die "final health/paper check failed"
