#!/usr/bin/env bash
#
# QAMC trading-utility recovery rollout — run ONCE, as root, from a root-owned 0700 copy.
#
# Derived from the externally reviewed ops/review/qamc-finish-line-rollout.sh
# (PR #48 / finish-line target). That script's Gate A/B/D/E machinery, rollback
# and self-integrity checks are reused verbatim. This version re-pins the
# baseline/target identity to the trading-utility recovery (PR #56, merged to
# main as d14e28d) and adds one new content-verification block, Phase 2b/C1b,
# naming the seven specific reviewed fixes so the deployed tree is proven to
# contain them, not just proven byte-identical to a reviewed hash.
#
# DO NOT copy this file by hand. Use the operator command in
# ops/review/README-recovery-rollout.md, which installs this exact reviewed
# blob root-owned 0700, verifies its sha256, and only then executes it.
#
# WHAT IT DOES — converges production from the deployed finish-line/PR #48
# target to the externally reviewed trading-utility recovery target (PR #56),
# re-establishes the already-authorized intraday opportunity scanner's local
# override (the committed default is still false; this is the same one-line
# delta production already carries), and runs the governed Stage E acceptance
# pass — in one run, in the docs/WORK.md order, with every stage gate an
# explicit fail-closed checkpoint. A later gate is unreachable unless the
# earlier ones passed on the deployed tree.
#
...
#            including the seven trading-utility recovery fix markers.
...
#   PHASE 6  GATE C — recovery fixes + inherited SGOV/Tech behaviour proven
#            on the DEPLOYED tree.
#   PHASE 7  GATE D — intraday market-data smoke, then re-enable the local
#            override (committed default is false).              [MUTATION]
...
BASELINE_SHA="775296e1d516279381a4c516dfb3e783b33a7495"   # current production / rollback point
TARGET_SHA="d14e28dfc63ca6e4da920229b0ab5ba0f33b93df"     # PR #56 merged to main
TARGET_TREE="7a795888f7794bbd7049ecd5468bf0aa3f419d86"    # tree of TARGET_SHA
EXPECTED_CHANGED_FILES=30
...
EXPECTED_FILELIST_SHA="c93082bd02df2e53439ffa7aabdd748d2b1312c5fd17acf4c2e5db24a58eb2af"
...
GATE_C_EXPECTED_TESTS=163    # exact; anything else is under- or over-collection
...
# leftovers cannot fake a pass. These five are INHERITED from the finish-line
# / PR #48 target (775296e1 is an ancestor of TARGET_SHA); they are regression
# checks proving the recovery branch did not revert them, not new evidence.
qgit "grep -q 'intraday_scan' ${TARGET_SHA} -- config/settings.yaml src/config.py" \
  || die "intraday_scan is absent from the target commit — this is not the expected lineage. STOP."
qgit "grep -q '_compute_deployable_cash' ${TARGET_SHA} -- src/pipeline.py" \
  || die "inherited SGOV deployable-cash fix absent from the target commit. STOP."
qgit "grep -qF 'return confirmed' ${TARGET_SHA} -- src/execution/cash_sweep.py" \
  || die "inherited cash-sweep confirmed-funding fix absent from the target commit. STOP."
qgit "grep -qF '_MAX_MISSING_RETRIES' ${TARGET_SHA} -- src/agents/tech_analyst.py" \
  || die "inherited Tech batch-completeness fix absent from the target commit. STOP."
qgit "grep -qF 'self._redact(exc)' ${TARGET_SHA} -- src/notifier.py" \
  || die "inherited Telegram token redaction absent from the target commit. STOP."
ok "all three inherited PR #48 fixes and the Telegram redaction still present in the target commit"

# ── Phase 2b. the seven trading-utility recovery fix markers (PR #56), NEW ──
# One grep per finding in docs/WORK.md's recovery record. Each is a stable
# identifier chosen so a rename/reformat is unlikely to false-negative, but a
# genuinely reverted fix — the actual failure mode this guards against — will
# always fail it, because the fix's naming and the failure it fixes are the
# same thing.
qgit "grep -qF '_strictly_inside' ${TARGET_SHA} -- src/agents/base.py" \
  || die "PM parse fix absent: nested-targets containment filter (_strictly_inside)
       not found in the target commit. STOP."
qgit "grep -qF '_FUND_TERMINAL_TIMEOUT_S' ${TARGET_SHA} -- src/execution/cash_sweep.py" \
  || die "SGOV funding-wait fix absent: bounded terminal wait constant not found
       in the target commit. STOP."
qgit "grep -qF 'repair_reprompt' ${TARGET_SHA} -- src/agents/risk_manager.py" \
  || die "PM/Risk schema-repair fix absent: repair_reprompt not found in
       risk_manager.py in the target commit. STOP."
qgit "grep -qF 'execution_skips' ${TARGET_SHA} -- src/pipeline_context.py" \
  || die "execution-skip telemetry fix absent: execution_skips not found in
       the target commit. STOP."
qgit "grep -qF 'buys_unfunded' ${TARGET_SHA} -- src/pipeline.py" \
  || die "unfunded-BUY retry fix absent: buys_unfunded status not found in
       the target commit. STOP."
qgit "grep -qF '_staleness_days' ${TARGET_SHA} -- src/data/macro.py" \
  || die "FRED cadence-aware staleness fix absent: _staleness_days not found
       in the target commit. STOP."
qgit "grep -qF '[constructor:' ${TARGET_SHA} -- src/portfolio_constructor.py" \
  || die "constructor sizing-cap provenance fix absent: '[constructor:' note
       not found in the target commit. STOP."
ok "all seven trading-utility recovery fix markers (PR #56) present in the target commit"
...
# ═════════════════════════════════════════════════════════════════════════════
say "PHASE 6 — GATE C: prove trading-utility recovery behaviour on the DEPLOYED tree"
# ═════════════════════════════════════════════════════════════════════════════
#
...
ok "C1 deployed tree == reviewed tree $TARGET_TREE (reviewed full suite: 1997 passed, 0 failed)"

# ── C1b. the seven recovery fix markers, re-checked on the deployed tree ────
# Phase 2b already fail-closed on these before any mutation; C1's tree-hash
# equality already proves the deployed content is byte-identical to what
# Phase 2b checked. This re-check is belt-and-suspenders in the same style as
# C4/C5 below, not new evidence.
qgit "grep -qF '_strictly_inside' ${TARGET_SHA} -- src/agents/base.py" \
  && qgit "grep -qF '_FUND_TERMINAL_TIMEOUT_S' ${TARGET_SHA} -- src/execution/cash_sweep.py" \
  && qgit "grep -qF 'repair_reprompt' ${TARGET_SHA} -- src/agents/risk_manager.py" \
  && qgit "grep -qF 'execution_skips' ${TARGET_SHA} -- src/pipeline_context.py" \
  ...
  && qgit "grep -qF '_staleness_days' ${TARGET_SHA} -- src/data/macro.py" \
  && qgit "grep -qF '[constructor:' ${TARGET_SHA} -- src/portfolio_constructor.py" \
  || abort "C1b: a trading-utility recovery fix marker present at Phase 2b is missing on the deployed tree"
ok "C1b all seven recovery fix markers confirmed on the deployed tree"

# ── C2. re-run the focused trading-utility recovery suites on the deployed commit ──
...
if [[ "$HAVE_PYTEST" -eq 1 ]]; then
  ...
  GATEC_OUT="$(as_qamc "cd '$EXPORTDIR' && '$VENV_PY' -m pytest \
      tests/test_cash_sweep.py tests/test_pipeline_stages.py \
      tests/test_pm_parse_fragment_incident.py tests/test_verdict_repair_reprompt.py \
      tests/test_execution_skip_telemetry.py tests/test_macro_analyst.py \
      tests/test_macro_data.py tests/test_risk_manager.py \
      tests/test_portfolio_constructor.py tests/test_api_funnel.py \
      -q -p no:randomly" 2>&1)"
  GATEC_RC=$?
  set -e
  ...
  if [[ "$GATEC_RC" -ne 0 ]]; then
    ...
    abort "C2: the trading-utility recovery focused suites FAILED on the deployed commit under the production venv"
  fi
...
' "$GATE_C_EXPECTED_TESTS" || abort "C2: focused-suite outcome is not exactly $GATE_C_EXPECTED_TESTS passed and nothing else"
  rm -rf "$EXPORTDIR"
  ok "C2 focused trading-utility recovery suites on the deployed commit, production venv: $GATEC_SUMMARY"
else
  ...
  note "     test_large_target_count_never_outranks_containing_object,"
  note "     test_fund_buys_slow_fill_confirmed_by_settle_poll,"
  note "     test_repair_that_flips_approved_fails_closed and"
  note "     test_buys_unfunded_is_retryable_in_main."
fi
...
# E12b — the seven trading-utility recovery fix markers, third and final check
qgit "grep -qF '_strictly_inside' HEAD -- src/agents/base.py" \
  && qgit "grep -qF '_FUND_TERMINAL_TIMEOUT_S' HEAD -- src/execution/cash_sweep.py" \
  && qgit "grep -qF 'repair_reprompt' HEAD -- src/agents/risk_manager.py" \
  && qgit "grep -qF 'execution_skips' HEAD -- src/pipeline_context.py" \
  && qgit "grep -qF 'buys_unfunded' HEAD -- src/pipeline.py" \
  && qgit "grep -qF '_staleness_days' HEAD -- src/data/macro.py" \
  && qgit "grep -qF '[constructor:' HEAD -- src/portfolio_constructor.py" \
  || abort "E12b: a trading-utility recovery fix marker is missing at the finish line"
ok "E12b all seven trading-utility recovery fix markers confirmed at the finish line"
...
cat <<EOF
  ...
  Recovery fixes        : all 7 PR #56 markers verified 3x (pre-checkout, post-checkout,
                          post-enable) — parse containment, funding wait/poll, schema
                          repair, skip telemetry, unfunded retry, FRED staleness, cap
                          provenance
